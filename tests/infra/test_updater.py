...
from geniriclaw.infra.updater import (
    UpdateObserver,
...
class TestPerformUpgradePipeline:
    ...
    async def test_changes_on_first_attempt(self) -> None:
        with (
            patch(
                "geniriclaw.infra.updater._perform_upgrade_impl",
                new=AsyncMock(return_value=(True, "first-pass")),
            ) as mock_upgrade,
            patch(
                "geniriclaw.infra.updater._wait_for_version_change",
    ...
    async def test_returns_unchanged_when_no_retry_target(self) -> None:
        with (
            ...
            patch(
                "geniriclaw.infra.updater._resolve_retry_target",
...
class TestUpdateObserver:
    ...
    async def test_notifies_on_new_version(self) -> None:
        ...
        with (
            patch("geniriclaw.infra.updater.check_pypi", return_value=info),
            patch("geniriclaw.infra.updater._INITIAL_DELAY_S", 0),
            patch("geniriclaw.infra.updater._CHECK_INTERVAL_S", 0.01),
        ):
            observer.start()
    ...
    async def test_handles_check_failure_gracefully(self) -> None:
        ...
        with (
            patch("geniriclaw.infra.updater.check_pypi", side_effect=RuntimeError("network")),
            patch("geniriclaw.infra.updater._INITIAL_DELAY_S", 0),
            patch("geniriclaw.infra.updater._CHECK_INTERVAL_S", 0.01),
        ):
            observer.start()
    ...
    async def test_handles_none_from_check_pypi(self) -> None:
        ...
        with (
            patch("geniriclaw.infra.updater.check_pypi", return_value=None),
            patch("geniriclaw.infra.updater._INITIAL_DELAY_S", 0),
            patch("geniriclaw.infra.updater._CHECK_INTERVAL_S", 0.01),
        ):
            observer.start()
    ...
    async def test_notifies_again_for_newer_version(self) -> None:
        ...
        with (
            patch("geniriclaw.infra.updater.check_pypi", side_effect=mock_check),
            patch("geniriclaw.infra.updater._INITIAL_DELAY_S", 0),
            patch("geniriclaw.infra.updater._CHECK_INTERVAL_S", 0.01),
        ):
            observer.start()
