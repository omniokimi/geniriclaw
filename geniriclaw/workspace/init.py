...
from geniriclaw.infra.atomic_io import atomic_text_save
from geniriclaw.workspace.cron_tasks import ensure_task_rule_files
from geniriclaw.workspace.paths import GeniriclawPaths
from geniriclaw.workspace.rules_selector import RulesSelector
from geniriclaw.workspace.skill_sync import sync_bundled_skills, sync_skills
...
def _sync_home_defaults(paths: GeniriclawPaths) -> None:
    """Walk the home-defaults template and copy to ``geniriclaw_home``.

    The template at ``<repo>/workspace/`` mirrors ``~/.geniriclaw/`` exactly.
    Zone rules per file:
    ...
    if not paths.home_defaults.is_dir():
        ...
        return
    _walk_and_copy(paths.home_defaults, paths.geniriclaw_home)
    ...
    if paths.geniriclaw_home.parent.name != "agents":
...
def _smart_merge_config(paths: GeniriclawPaths) -> None:
...
def _migrate_tasks_to_cron_tasks(paths: GeniriclawPaths) -> None:
...
def _clean_orphan_symlinks(paths: GeniriclawPaths) -> None:
...
def _ensure_required_dirs(paths: GeniriclawPaths) -> None:
    """Create any required directories that are missing."""
    for rel in _REQUIRED_DIRS:
        d = paths.geniriclaw_home / rel
        if not d.is_dir():
            d.mkdir(parents=True, exist_ok=True)
            logger.info("Created missing directory: %s", d)


def init_workspace(paths: GeniriclawPaths) -> None:
    """Initialize the workspace: defaults sync, rule sync, config merge, cleanup."""
    logger.info("Workspace init started home=%s", paths.geniriclaw_home)
...
- Your filesystem is isolated. `/geniriclaw` is the mounted host directory `~/.geniriclaw`.
