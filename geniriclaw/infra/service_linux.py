"""Systemd user service management for geniriclaw (Linux)."""
...
from geniriclaw.infra.service_logs import print_journal_service_logs
...
def _generate_service_unit(binary_path: str) -> str:
    ...
    return f"""\
[Unit]
Description=geniriclaw - Telegram bot powered by AI CLIs
...
Environment=GENIRICLAW_SUPERVISOR=1
...
def is_service_installed() -> bool:
    """Check if the geniriclaw service is installed."""
    return _service_path().exists()


def is_service_running() -> bool:
    """Check if the geniriclaw service is currently running."""
...
def install_service(console: Console | None = None) -> bool:
    """Install and start the geniriclaw systemd user service.
...
def uninstall_service(console: Console | None = None) -> bool:
    """Stop, disable, and remove the geniriclaw service."""
    console = ensure_console(console)
