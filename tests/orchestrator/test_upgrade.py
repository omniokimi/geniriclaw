...
from geniriclaw.orchestrator.commands import cmd_upgrade
...
class TestCmdUpgrade:
    ...
    async def test_shows_update_with_buttons(self, orch: Orchestrator) -> None:
        ...
        with (
            ...
            patch("geniriclaw.orchestrator.commands.check_pypi", return_value=info),
        ):
    ...
    async def test_handles_pypi_failure(self, orch: Orchestrator) -> None:
        with (
            patch("geniriclaw.infra.install.detect_install_mode", return_value="pipx"),
            patch("geniriclaw.orchestrator.commands.check_pypi", return_value=None),
        ):
            result = await cmd_upgrade(orch, 1, "/upgrade")
