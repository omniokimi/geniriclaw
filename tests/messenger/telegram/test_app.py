...
from geniriclaw.config import AgentConfig, StreamingConfig
...
def _make_tg_bot(
    ...
    *bot_instance* is the mocked aiogram Bot for assertion.
    """
    from geniriclaw.messenger.telegram.app import TelegramBot
    ...
    with patch("geniriclaw.messenger.telegram.app.Bot", return_value=bot_instance):
...
def _make_orchestrator(
    *,
    ...
    paths.geniriclaw_home = Path("/tmp/test-geniriclaw")
...
class TestCancelTask:
    async def test_cancel_task_none(self) -> None:
        from geniriclaw.messenger.telegram.app import _cancel_task
...
class TestOnHelp:
    @patch("geniriclaw.messenger.telegram.app.send_rich", new_callable=AsyncMock)
    ...
    async def test_help_lists_all_registered_commands(self, mock_send: AsyncMock) -> None:
        from geniriclaw.commands import BOT_COMMANDS
...
class TestOnStart:
    @patch("geniriclaw.messenger.telegram.app.send_rich", new_callable=AsyncMock)
    @patch("geniriclaw.messenger.telegram.app.build_welcome_keyboard")
    @patch("geniriclaw.messenger.telegram.app.build_welcome_text", return_value="Welcome!")
    @patch("geniriclaw.cli.auth.check_all_auth", return_value={})
...
class TestSendWelcomeImage:
    async def test_returns_false_when_no_image_file(self) -> None:
        ...
        with patch(
            "geniriclaw.messenger.telegram.app._WELCOME_IMAGE", Path("/nonexistent/file.png")
        ):
    ...
    async def test_returns_true_with_short_caption(self) -> None:
        ...
        with (
            patch("geniriclaw.messenger.telegram.app._WELCOME_IMAGE") as mock_path,
            patch(
                "geniriclaw.messenger.telegram.app.markdown_to_telegram_html",
                return_value="<b>Hi</b>",
            ),
        ):
    ...
    async def test_returns_false_on_bad_request(self) -> None:
        ...
        with patch("geniriclaw.messenger.telegram.app._WELCOME_IMAGE") as mock_path:
            mock_path.is_file.return_value = True
...
class TestOnRestart:
    ...
    async def test_restart_writes_sentinel_and_stops(self, mock_send: AsyncMock) -> None:
        ...
        with patch("geniriclaw.infra.restart.write_restart_sentinel") as mock_sentinel:
...
class TestOnMessage:
    async def test_routes_text_to_non_streaming(self) -> None:
        ...
        with patch(
            "geniriclaw.messenger.telegram.app.run_non_streaming_message", new_callable=AsyncMock
        ) as mock_run:
    ...
    @patch("geniriclaw.messenger.telegram.app.strip_mention", return_value="clean text")
...
class TestResolveText:
    ...
    @patch("geniriclaw.messenger.telegram.app.resolve_media_text", new_callable=AsyncMock)
    @patch("geniriclaw.messenger.telegram.app.has_media", return_value=True)
    ...
    @patch("geniriclaw.messenger.telegram.app.is_media_addressed", return_value=False)
    @patch("geniriclaw.messenger.telegram.app.has_media", return_value=True)
    ...
    @patch("geniriclaw.messenger.telegram.app.resolve_media_text", new_callable=AsyncMock)
    @patch("geniriclaw.messenger.telegram.app.is_media_addressed", return_value=True)
    @patch("geniriclaw.messenger.telegram.app.has_media", return_value=True)
    ...
    @patch("geniriclaw.messenger.telegram.app.is_command_for_others", return_value=True)
    @patch("geniriclaw.messenger.telegram.app.has_media", return_value=True)
...
class TestHandleStreaming:
    async def test_streaming_fallback_sends_rich(self) -> None:
        ...
        with patch(
            "geniriclaw.messenger.telegram.app.run_streaming_message", new_callable=AsyncMock
        ) as mock_run:
...
class TestCallbackQueryHandler:
    ...
    async def test_model_selector_callback_edits_message(self) -> None:
        ...
        from geniriclaw.orchestrator.selectors.models import (
            Button,
            ButtonGrid,
            SelectorResponse,
        )
        ...
        with patch(
            "geniriclaw.orchestrator.selectors.model_selector.handle_model_callback",
    ...
    async def test_cron_selector_callback_edits_message(self) -> None:
        ...
        with patch(
            "geniriclaw.orchestrator.selectors.cron_selector.handle_cron_callback",
            new_callable=AsyncMock,
            return_value=resp,
        ):
    ...
    async def test_welcome_callback_resolves_to_prompt(self) -> None:
        ...
        assert "set up geniriclaw.dev" in sent_text
...
class TestHandleModelSelector:
    ...
    async def test_suppresses_bad_request(self) -> None:
        from geniriclaw.orchestrator.selectors.models import SelectorResponse
        from geniriclaw.session.key import SessionKey
...
class TestCommandHandlers:
    @patch(
        "geniriclaw.messenger.telegram.app.handle_abort", new_callable=AsyncMock, return_value=True
    )
    ...
    @patch("geniriclaw.messenger.telegram.app.handle_command", new_callable=AsyncMock)
    ...
    @patch("geniriclaw.messenger.telegram.app.handle_new_session", new_callable=AsyncMock)
...
class TestWebhookWake:
    async def test_calls_handle_message_and_sends_result(self) -> None:
        import geniriclaw.messenger.telegram.transport as _tgt
...
class TestWatchRestartMarker:
    async def test_detects_marker_and_stops(self, tmp_path: Path) -> None:
        ...
        tg_bot, _ = _make_tg_bot()
        orch = _make_orchestrator()
        orch.paths.geniriclaw_home = tmp_path
        tg_bot._orchestrator = orch
...
class TestSyncCommands:
    async def test_sets_commands_when_different(self) -> None:
        from geniriclaw.messenger.telegram.app import _BOT_COMMANDS
...
class TestForumTopicPropagation:
    ...
    async def test_on_restart_passes_thread_id(self, mock_send: AsyncMock) -> None:
        ...
        with patch("geniriclaw.infra.restart.write_restart_sentinel"):
            await tg_bot._on_restart(msg)
