...
geniriclaw's primary transport is Telegram. Matrix is an optional second transport you can add at any time — either as the only transport or running alongside Telegram.
...
```bash
# pipx (recommended)
geniriclaw install matrix

# pip
pip install "geniriclaw[matrix]"
...
| User ID | `@my_geniriclaw_bot:matrix.org` |
...
Edit `~/.geniriclaw/config/config.json`:
...
```json
{
  ...
  "matrix": {
    ...
    "user_id": "@my_geniriclaw_bot:matrix.org",
...
On first start, geniriclaw logs in with the password and saves credentials to `~/.geniriclaw/matrix_store/credentials.json`. After that, the password is no longer needed — token-based auth is used automatically.
...
| `store_path` | no | Relative to geniriclaw_home. Default: `matrix_store` |
...
| Sub-agent setup | `geniriclaw agents add` (interactive) | Manual via `agents.json` |
...
- Run `geniriclaw install matrix` to install the dependency
...
- Delete `~/.geniriclaw/matrix_store/credentials.json` and restart — geniriclaw will re-login with password
