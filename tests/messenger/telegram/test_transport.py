...
from geniriclaw.bus.cron_sanitize import (
...
class TestCronBroadcast:
    async def test_broadcasts_with_result_text(self) -> None:
        ...
        with patch("geniriclaw.messenger.telegram.transport.send_rich", new_callable=AsyncMock):
...
class TestHeartbeatDelivery:
    async def test_delivers_to_chat(self) -> None:
        ...
        with patch(
            "geniriclaw.messenger.telegram.transport.send_rich", new_callable=AsyncMock
        ) as mock_send:
