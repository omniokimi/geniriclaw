...
from geniriclaw.cli.auth import AuthStatus, check_claude_auth, check_codex_auth, check_gemini_auth
from geniriclaw.config import DEFAULT_EMPTY_GEMINI_API_KEY, AgentConfig, deep_merge_config
from geniriclaw.i18n import t_rich
from geniriclaw.workspace.init import init_workspace
...
def _build_extras_table(console: Console) -> None:
    ...
    from geniriclaw.infra.docker_extras import DOCKER_EXTRAS_BY_ID, extras_for_display
...
def _ask_docker_extras(console: Console) -> list[str]:
    ...
    from geniriclaw.infra.docker_extras import (
        DOCKER_EXTRAS_BY_ID,
        extras_for_display,
        resolve_extras,
    )
...
def _offer_service_install(console: Console) -> bool:
    """Ask whether to install geniriclaw as a background service."""
    from geniriclaw.infra.service import is_service_available
...
def run_onboarding() -> bool:
    ...
    console.print(
        Panel(
            ...
            + "\n\n"
            + t_rich("wizard.complete.home", path=paths.geniriclaw_home)
            + "\n"
    ...
    if run_as_service:
        from geniriclaw.infra.service import install_service
...
def run_smart_reset(geniriclaw_home: Path) -> None:
    """Read existing config, handle Docker cleanup, and delete workspace."""
    console = Console()
    console.print()

    config_path = geniriclaw_home / "config" / "config.json"
    ...
    if config_path.exists():
        try:
            ...
            if isinstance(docker, dict) and docker.get("enabled"):
                docker_container = str(docker.get("container_name", "geniriclaw-sandbox"))
                docker_image = str(docker.get("image_name", "geniriclaw-sandbox"))
        except (json.JSONDecodeError, OSError):
            pass
    ...
    console.print(
        Panel(
            t_rich("wizard.reset.body", home=geniriclaw_home),
    ...
    if geniriclaw_home.exists():
        console.print(t_rich("wizard.reset.confirm.warning", home=geniriclaw_home) + "\n")
    else:
