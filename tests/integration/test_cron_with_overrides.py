...
from geniriclaw.cli.param_resolver import TaskOverrides, resolve_cli_config
from geniriclaw.config import AgentConfig
from geniriclaw.cron.execution import build_cmd
...
def mock_paths(tmp_path: Path) -> GeniriclawPaths:
    """Create mock GeniriclawPaths for testing."""
    home = tmp_path / "geniriclaw_home"
    fw = tmp_path / "fw"
...
async def test_cron_task_model_override(
    ...
    # Build command
    with patch("geniriclaw.cron.execution.which", return_value="/usr/bin/claude"):
...
async def test_cron_task_cli_parameters(
    ...
    # Build command
    with patch("geniriclaw.cron.execution.which", return_value="/usr/bin/codex"):
        result = build_cmd(exec_config, "Test prompt")
