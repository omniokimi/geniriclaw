...
from geniriclaw.cli.codex_discovery import CodexModelInfo
from geniriclaw.config import AgentConfig
from geniriclaw.cron.execution import (
    enrich_instruction,
    parse_claude_result,
    parse_codex_result,
)
...
def _make_manager(paths: GeniriclawPaths) -> CronManager:
...
def _write_jobs(paths: GeniriclawPaths, jobs: list[CronJob]) -> None:
...
class TestCronObserverScheduling:
    ...
    async def test_request_reschedule_coalesces_pending_task(self, tmp_path: Path) -> None:
        ...
        with patch(
            "geniriclaw.cron.observer.asyncio.create_task", side_effect=_create_task
        ) as create:
...
class TestCronObserverExecution:
    ...
    async def test_handles_missing_cli(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("geniriclaw.cron.execution.which", return_value=None),
        ):
    ...
    async def test_executes_claude_job_success(self, tmp_path: Path) -> None:
        ...
        with (
            time_machine.travel(datetime(2026, 1, 15, 14, 0, tzinfo=UTC)),
            patch("geniriclaw.cron.execution.which", return_value="/usr/bin/claude"),
    ...
    async def test_executes_codex_job_success(self, tmp_path: Path) -> None:
        ...
        with (
            time_machine.travel(datetime(2026, 1, 15, 14, 0, tzinfo=UTC)),
            patch("geniriclaw.cron.execution.which", return_value="/usr/bin/codex"),
            patch("asyncio.create_subprocess_exec", return_value=mock_proc) as exec_mock,
        ):
            await observer._execute_job("daily", "Generate report", "daily")
...
class TestCronResultDelivery:
    ...
    async def test_run_at_catches_unexpected_exception(self, tmp_path: Path) -> None:
        ...
        from geniriclaw.cron.observer import _ScheduledJob
