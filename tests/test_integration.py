...
def _setup_framework(fw_root: Path) -> None:
    ...
    (ws / "CLAUDE.md").write_text("# Geniriclaw Home")
...
@pytest.fixture
def workspace(tmp_path: Path) -> tuple[GeniriclawPaths, AgentConfig]:
    fw_root = tmp_path / "fw"
    _setup_framework(fw_root)
    paths = GeniriclawPaths(
        geniriclaw_home=tmp_path / "home", home_defaults=fw_root / "workspace", framework_root=fw_root
    )
...
def orch_with_mock_cli(
    workspace: tuple[GeniriclawPaths, AgentConfig],
...
class TestCommandRouting:
    async def test_status_command(self, orch_with_mock_cli: tuple[Orchestrator, AsyncMock]) -> None:
        ...
        with patch("geniriclaw.orchestrator.commands.check_all_auth", return_value={}):
    ...
    async def test_model_command_no_args_returns_selector(
        ...
        with patch(
            "geniriclaw.orchestrator.commands.model_selector_start",
...
class TestSessionPersistence:
    async def test_sessions_survive_manager_recreation(
        self, workspace: tuple[GeniriclawPaths, AgentConfig]
    ) -> None:
        paths, config = workspace
    ...
    async def test_session_json_is_valid(self, workspace: tuple[GeniriclawPaths, AgentConfig]) -> None:
        paths, config = workspace
