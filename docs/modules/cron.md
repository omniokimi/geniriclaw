...
File: `~/.geniriclaw/cron_jobs.json`
...
- `GENIRICLAW_CHAT_ID` -- current chat ID (set when `chat_id` is non-zero)
- `GENIRICLAW_TOPIC_ID` -- current topic ID (set when `topic_id` is non-None)
- `GENIRICLAW_TRANSPORT` -- transport identifier (`"tg"` or `"mx"`)
