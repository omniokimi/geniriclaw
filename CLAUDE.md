...
geniriclaw is a multi-transport chat orchestrator for the official provider CLIs (`claude`, `codex`, `gemini`).
It runs Telegram and/or Matrix, can expose an optional direct WebSocket API, keeps state under `~/.geniriclaw`, and supervises the main agent plus optional sub-agents in one asyncio process.
...
| `workspace/` | `~/.geniriclaw` path model, seeding, rule deployment/sync, skill sync |
...
- `GeniriclawPaths` in `workspace/paths.py` is the single source of truth for runtime paths.
...
- Skill sync spans `~/.geniriclaw/workspace/skills`, `~/.claude/skills`, `~/.codex/skills`, `~/.gemini/skills`:
  ...
  - Docker mode: managed copies (`.geniriclaw_managed`)
...
- `geniriclaw`, `geniriclaw onboarding`, `geniriclaw reset`
- `geniriclaw status`, `geniriclaw stop`, `geniriclaw restart`, `geniriclaw upgrade`, `geniriclaw uninstall`
