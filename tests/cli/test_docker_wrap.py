...
from geniriclaw.cli.base import CLIConfig, docker_wrap
...
def test_docker_wrap_with_container() -> None:
    ...
    assert result_cmd == [
        ...
        "-e",
        "GENIRICLAW_CHAT_ID=42",
        "-e",
        "GENIRICLAW_TRANSPORT=tg",
        "-e",
        "GENIRICLAW_AGENT_NAME=main",
        "-e",
        "GENIRICLAW_INTERAGENT_PORT=8799",
        "-e",
        "GENIRICLAW_HOME=/geniriclaw",
        "-e",
        "GENIRICLAW_SHARED_MEMORY_PATH=/geniriclaw/SHAREDMEMORY.md",
        "-e",
        "GENIRICLAW_INTERAGENT_HOST=host.docker.internal",
        "my-sandbox",
        "claude",
        "-p",
        "hello",
...
def test_docker_wrap_injects_chat_id() -> None:
    ...
    result_cmd, _ = docker_wrap(cmd, cfg)
    assert "GENIRICLAW_CHAT_ID=999" in result_cmd
...
def test_docker_wrap_sub_agent_container_paths() -> None:
    """Sub-agent working_dir maps to /geniriclaw/agents/<name>/workspace inside container."""
    cmd = ["claude", "-p", "hi"]
    cfg = CLIConfig(
        docker_container="sandbox",
        chat_id=1,
        working_dir="/home/user/.geniriclaw/agents/test/workspace",
        agent_name="test",
    )
    result_cmd, cwd = docker_wrap(cmd, cfg)
    assert cwd is None
    ...
    assert result_cmd[w_idx + 1] == "/geniriclaw/agents/test/workspace"
    # GENIRICLAW_HOME is the sub-agent home inside the container
    assert "GENIRICLAW_HOME=/geniriclaw/agents/test" in result_cmd
    # Shared memory is at the root
    assert "GENIRICLAW_SHARED_MEMORY_PATH=/geniriclaw/SHAREDMEMORY.md" in result_cmd


def test_docker_wrap_main_agent_container_paths() -> None:
    """Main agent working_dir maps to /geniriclaw/workspace inside container."""
    cmd = ["claude", "-p", "hi"]
    cfg = CLIConfig(
        docker_container="sandbox",
        chat_id=1,
        working_dir="/home/user/.geniriclaw/workspace",
        agent_name="main",
    )
    result_cmd, _ = docker_wrap(cmd, cfg)
    w_idx = result_cmd.index("-w")
    assert result_cmd[w_idx + 1] == "/geniriclaw/workspace"
    assert "GENIRICLAW_HOME=/geniriclaw" in result_cmd
    assert "GENIRICLAW_SHARED_MEMORY_PATH=/geniriclaw/SHAREDMEMORY.md" in result_cmd
...
def test_docker_wrap_sub_agent_windows_paths_are_posix() -> None:
    ...
    cfg = CLIConfig(
        docker_container="sandbox",
        chat_id=1,
        working_dir=PureWindowsPath(r"C:\Users\me\.geniriclaw\agents\seismic-bot\workspace"),
        agent_name="seismic-bot",
    )
    result_cmd, _ = docker_wrap(cmd, cfg, interactive=True)
    w_idx = result_cmd.index("-w")
    assert result_cmd[w_idx + 1] == "/geniriclaw/agents/seismic-bot/workspace"
    assert "GENIRICLAW_HOME=/geniriclaw/agents/seismic-bot" in result_cmd
