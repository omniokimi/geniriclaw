...
from geniriclaw.cleanup.observer import CleanupObserver, _delete_old_files
from geniriclaw.config import AgentConfig, CleanupConfig
...
def _make_paths(tmp_path: Path) -> GeniriclawPaths:
    return GeniriclawPaths(geniriclaw_home=tmp_path)
...
async def test_maybe_run_skips_wrong_hour(tmp_path: Path) -> None:
    ...
    with patch("geniriclaw.cleanup.observer.datetime") as mock_dt:
