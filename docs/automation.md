...
- persisted in `~/.geniriclaw/tasks.json`
- task folders in `~/.geniriclaw/workspace/tasks/<task_id>/`
...
- forum-topic tasks route back to the originating topic via `thread_id` / `GENIRICLAW_TOPIC_ID`

## Cron jobs

Cron jobs run in `~/.geniriclaw/workspace/cron_tasks/<task_folder>/`.
...
```text
~/.geniriclaw/workspace/cron_tasks/weather-report/
  CLAUDE.md
  AGENTS.md
  TASK_DESCRIPTION.md
