...
Environment variable GENIRICLAW_TASK_ID is automatically set by the framework
...
def main() -> None:
    ...
    question = args[0]
    task_id = os.environ.get("GENIRICLAW_TASK_ID", "")

    if not task_id:
        print(
            "Error: GENIRICLAW_TASK_ID not set. This tool can only be used inside a background task.",
            file=sys.stderr,
        )
        sys.exit(1)
