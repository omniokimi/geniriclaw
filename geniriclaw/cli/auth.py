...
from geniriclaw.cli.gemini_utils import find_gemini_cli
...
def _gemini_key_auth_source(gemini_home: Path) -> tuple[Path | None, datetime | None]:
    ...
    geniriclaw_key, geniriclaw_config_path = read_geniriclaw_gemini_api_key()
    if geniriclaw_key and geniriclaw_config_path is not None:
        return geniriclaw_config_path, datetime.fromtimestamp(
            geniriclaw_config_path.stat().st_mtime, tz=UTC
        )
    return None, None
...
def read_geniriclaw_gemini_api_key() -> tuple[str | None, Path | None]:
    """Read ``gemini_api_key`` from ``~/.geniriclaw/config/config.json``.

    Returns ``(key, path)`` when configured, otherwise ``(None, None)``.
    """
    config_path = _geniriclaw_config_path()
...
def _geniriclaw_config_path() -> Path:
