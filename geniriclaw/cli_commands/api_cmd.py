"""API server management CLI subcommands (``geniriclaw api ...``)."""
...
from geniriclaw.config import _BIND_ALL_INTERFACES
...
def print_api_help() -> None:
    ...
    table.add_column()
    table.add_row("geniriclaw api enable", "Enable the WebSocket API server")
    table.add_row("geniriclaw api disable", "Disable the WebSocket API server")
...
def api_install_hint() -> str:
    ...
    if mode == "pipx":
        return "pipx inject geniriclaw PyNaCl"
    return "pip install geniriclaw[api]"


def api_enable() -> None:
    """Enable the API server: check deps, write config, generate token."""
    from geniriclaw.cli_commands.docker import docker_read_config
...
def cmd_api(args: list[str]) -> None:
    """Handle 'geniriclaw api <subcommand>'."""
