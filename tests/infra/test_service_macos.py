...
from geniriclaw.infra.service_macos import (
    _LABEL,
...
class TestGeneratePlistData:
    def test_contains_binary_path(self) -> None:
        data = _generate_plist_data("/usr/local/bin/geniriclaw")
        assert data["ProgramArguments"] == ["/usr/local/bin/geniriclaw"]

    def test_has_label(self) -> None:
        data = _generate_plist_data("geniriclaw")
        assert data["Label"] == "dev.geniri.claw"
    ...
    def test_path_includes_all_nvm_bins(self, tmp_path: Path) -> None:
        ...
        with patch("geniriclaw.infra.service_macos.Path.home", return_value=tmp_path):
            data = _generate_plist_data("geniriclaw")
    ...
    def test_generates_valid_plist(self) -> None:
        ...
        assert parsed["Label"] == "dev.geniri.claw"
        assert parsed["RunAtLoad"] is True


class TestIsServiceInstalled:
    @patch("geniriclaw.infra.service_macos._plist_path")
...
class TestIsServiceRunning:
    @patch("geniriclaw.infra.service_macos.is_service_installed", return_value=False)
    def test_not_running_when_not_installed(self, _mock: MagicMock) -> None:
        assert is_service_running() is False

    @patch("geniriclaw.infra.service_macos._run_launchctl")
    @patch("geniriclaw.infra.service_macos.is_service_installed", return_value=True)
    def test_running_when_pid_present(self, _installed: MagicMock, mock_run: MagicMock) -> None:
        mock_run.return_value = make_completed(
            0,
            stdout='{\n\t"PID" = 12345;\n\t"Label" = "dev.geniri.claw";\n};',
        )
...
class TestInstallService:
    @patch("geniriclaw.infra.service_macos._run_launchctl")
    @patch("geniriclaw.infra.service_macos.is_service_installed", return_value=False)
    @patch("geniriclaw.infra.service_macos.is_service_available", return_value=True)
    @patch(
        "geniriclaw.infra.service_macos.find_geniriclaw_binary", return_value="/usr/local/bin/geniriclaw"
    )
    @patch("geniriclaw.infra.service_macos._plist_path")
    @patch("geniriclaw.infra.service_macos.resolve_paths")
    def test_install_success(
        ...
        plist_file = tmp_path / "dev.geniri.claw.plist"
        ...
        assert plist_data["Label"] == "dev.geniri.claw"

    @patch("geniriclaw.infra.service_macos.is_service_available", return_value=False)
    def test_install_fails_without_launchctl(self, _avail: MagicMock) -> None:
        console = MagicMock()
        assert install_service(console) is False

    @patch("geniriclaw.infra.service_macos.is_service_available", return_value=True)
    @patch("geniriclaw.infra.service_macos.find_geniriclaw_binary", return_value=None)
...
class TestStopService:
    @patch("geniriclaw.infra.service_macos._run_launchctl")
    @patch("geniriclaw.infra.service_macos.is_service_running", return_value=True)
    ...
    @patch("geniriclaw.infra.service_macos.is_service_running", return_value=False)
...
class TestPrintServiceLogs:
    ...
    def test_shows_logs_from_file(
        ...
        logs_dir.mkdir()
        log_file = logs_dir / "geniriclaw_2026-02-22.log"
