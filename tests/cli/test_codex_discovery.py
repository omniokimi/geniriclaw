...
from geniriclaw.cli.codex_discovery import CodexModelInfo, discover_codex_models

_INIT_RESPONSE = json.dumps(
    {
        "id": 1,
        "result": {"userAgent": "geniriclaw/0.98.0"},
    }
)
...
async def test_discover_models_parses_response() -> None:
    proc = _mock_process()
    with (
        patch("geniriclaw.cli.codex_discovery.which", return_value="/usr/bin/codex"),
...
async def test_discover_models_codex_not_installed() -> None:
    with patch("geniriclaw.cli.codex_discovery.which", return_value=None):
        models = await discover_codex_models()
    assert models == []
