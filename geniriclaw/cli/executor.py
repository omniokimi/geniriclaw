...
from geniriclaw.cli.base import (
    _IS_WINDOWS,
    CLIConfig,
    _win_feed_stdin,
)
from geniriclaw.cli.stream_events import ResultEvent, StreamEvent
from geniriclaw.cli.timeout_controller import TimeoutController
from geniriclaw.cli.types import CLIResponse
from geniriclaw.infra.platform import CREATION_FLAGS as _CREATION_FLAGS
from geniriclaw.infra.process_tree import force_kill_process_tree
...
def _build_subprocess_env(config: CLIConfig) -> dict[str, str] | None:
    ...
    agent identification variables.  User secrets from ``~/.geniriclaw/.env`` are
    merged in without overriding existing variables.
    """
    import os
    from pathlib import Path

    from geniriclaw.infra.env_secrets import load_env_secrets

    env = os.environ.copy()

    # Merge user secrets (low priority — never override existing vars).
    working_dir = Path(config.working_dir)
    geniriclaw_home = working_dir.parent if working_dir.name == "workspace" else working_dir
    env_file = geniriclaw_home / ".env"
    for key, value in load_env_secrets(env_file).items():
        if key not in env:
            env[key] = value

    env["GENIRICLAW_AGENT_NAME"] = config.agent_name
    env["GENIRICLAW_AGENT_ROLE"] = "main" if config.agent_name == "main" else "sub"
    env["GENIRICLAW_INTERAGENT_PORT"] = str(config.interagent_port)
    if config.chat_id:
        env["GENIRICLAW_CHAT_ID"] = str(config.chat_id)
    if config.topic_id:
        env["GENIRICLAW_TOPIC_ID"] = str(config.topic_id)
    env["GENIRICLAW_TRANSPORT"] = config.transport
    working_dir = Path(config.working_dir)
    geniriclaw_home = working_dir.parent if working_dir.name == "workspace" else working_dir
    env["GENIRICLAW_HOME"] = str(geniriclaw_home)
    # Shared knowledge is always at the main agent's home level.
    # For main: geniriclaw_home itself. For sub-agents: ../../ from agents/<name>/.
    if config.agent_name == "main":
        env["GENIRICLAW_SHARED_MEMORY_PATH"] = str(geniriclaw_home / "SHAREDMEMORY.md")
    else:
        # Sub-agent home is <main_home>/agents/<name>/
        main_home = geniriclaw_home.parent.parent
        env["GENIRICLAW_SHARED_MEMORY_PATH"] = str(main_home / "SHAREDMEMORY.md")
    return env
