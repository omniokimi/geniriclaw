...
class TestDockerManagerMounts:
    ...
    async def test_single_mount_in_run_cmd(self, tmp_path: Path, docker_paths: GeniriclawPaths) -> None:
        proj = tmp_path / "myapp"
        proj.mkdir()
    ...
    async def test_multiple_mounts_in_run_cmd(
        self, tmp_path: Path, docker_paths: GeniriclawPaths
    ) -> None:
    ...
    async def test_empty_mounts_list(self, docker_paths: GeniriclawPaths) -> None:
...
class TestDockerMountCLI:
    ...
    def test_mount_adds_path(self, tmp_path: Path) -> None:
        from geniriclaw.cli_commands.docker import docker_mount
        ...
        with patch("geniriclaw.cli_commands.docker.resolve_paths", return_value=paths_mock):
    ...
    def test_unmount_removes_path(self, tmp_path: Path) -> None:
        from geniriclaw.cli_commands.docker import docker_unmount
    ...
    def test_mounts_list_empty(self, tmp_path: Path) -> None:
        from geniriclaw.cli_commands.docker import docker_list_mounts
...
class TestConfigDeepMergeMounts:
    ...
    def test_new_mounts_key_added(self) -> None:
        from geniriclaw.config import deep_merge_config
