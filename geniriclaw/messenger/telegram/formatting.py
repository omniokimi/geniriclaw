...
from geniriclaw.messenger.telegram.buttons import strip_button_syntax
...
def markdown_to_telegram_html(text: str) -> str:
    ...
    # URLs wrapped in backticks should render as clickable links, not as <code>.
    # In Telegram <code> is tap-to-copy; users want tap-to-open for URLs.
    url_links: list[tuple[str, str]] = []
    _URL_ONLY_RE = re.compile(r"^\s*(https?://\S+?)\s*$")

    def _save_inline_code(m: re.Match[str]) -> str:
        content = m.group(1)
        url_match = _URL_ONLY_RE.match(content)
        if url_match:
            idx = len(url_links)
            url = url_match.group(1)
            url_links.append((url, url))
            return _placeholder("UL", idx)
    ...
    # Autolink bare URLs (not already inside <a href=...> or href="...).
    # Telegram clients usually detect bare URLs too, but explicit <a> is more
    # reliable and renders with link styling immediately.
    ...
    for i, (href, label) in enumerate(url_links):
        text = text.replace(_placeholder("UL", i), f'<a href="{href}">{html.escape(label)}</a>')
