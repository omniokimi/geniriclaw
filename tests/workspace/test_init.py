...
from geniriclaw.workspace.init import init_workspace, inject_runtime_environment
...
def _setup_home_defaults(fw_root: Path) -> None:
    ...
    Mirrors the repo ``workspace/`` structure (1:1 copy of ~/.geniriclaw/).
    """
    ws = fw_root / "workspace"

    # Top-level CLAUDE.md (geniriclaw home)
    ws.mkdir(parents=True)
    (ws / "CLAUDE.md").write_text("# Geniriclaw Home CLAUDE.md")
...
def test_geniriclaw_home_claude_md_overwritten(tmp_path: Path) -> None:
    """The geniriclaw_home/CLAUDE.md is Zone 2 (always overwritten)."""
    paths = _make_paths(tmp_path)
    init_workspace(paths)

    home_claude = paths.geniriclaw_home / "CLAUDE.md"
    assert home_claude.exists()
    assert home_claude.read_text() == "# Geniriclaw Home CLAUDE.md"
...
def test_inject_docker_notice(tmp_path: Path) -> None:
    paths = _make_paths(tmp_path)
    init_workspace(paths)
    inject_runtime_environment(paths, docker_container="geniriclaw-sandbox")
    content = (paths.workspace / "CLAUDE.md").read_text()
    assert "DOCKER CONTAINER" in content
    assert "geniriclaw-sandbox" in content
