...
- `GENIRICLAW_CHAT_ID`
- `GENIRICLAW_TOPIC_ID` (when set)
- `GENIRICLAW_TRANSPORT` (active transport identifier, e.g. `"tg"`, `"mx"`)
...
- may inject `GEMINI_API_KEY` from geniriclaw config when Gemini settings indicate API-key mode and no env key is set
...
- Gemini:
  ...
  - optional fallback to `~/.geniriclaw/config/config.json` `gemini_api_key`

## Model caches

### Codex cache

- file: `~/.geniriclaw/config/codex_models.json`
...
- file: `~/.geniriclaw/config/gemini_models.json`
...
- container mode:
  ...
  - injects `GENIRICLAW_CHAT_ID`, optional `GENIRICLAW_TOPIC_ID`, `GENIRICLAW_TRANSPORT`, `GENIRICLAW_AGENT_NAME`, `GENIRICLAW_INTERAGENT_PORT`, `GENIRICLAW_HOME`, `GENIRICLAW_SHARED_MEMORY_PATH`, and `GENIRICLAW_INTERAGENT_HOST`,
  - merges user secrets from `~/.geniriclaw/.env` (never overrides existing vars),
