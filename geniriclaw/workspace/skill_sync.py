"""Cross-platform skill directory sync between geniriclaw workspace and CLI tools.

Provides multi-way symlink synchronization so skills installed via Claude Code,
Codex CLI, Gemini CLI, or the geniriclaw workspace are visible to all agents.

Includes bundled-skill linking (package → workspace), sync-time external-symlink
protection, and cleanup of geniriclaw-created links on shutdown.

When Docker sandboxing is active, symlinks are replaced with directory copies
(marked with ``.geniriclaw_managed``) because absolute host paths do not resolve
...
_MANAGED_MARKER = ".geniriclaw_managed"
...
def _resolve_canonical(
    ...
    *registries: dict[str, Path],
    ...
    Priority follows argument order (typically geniriclaw > claude > codex > gemini).
...
def _is_managed_copy(path: Path) -> bool:
    """Return ``True`` if *path* is a geniriclaw-managed copy (has marker file)."""
...
def _ensure_copy(dest: Path, source: Path) -> bool:
    """Copy *source* directory to *dest* with a ``.geniriclaw_managed`` marker.
...
def sync_skills(paths: GeniriclawPaths, *, docker_active: bool = False) -> None:
    """Multi-way skill directory sync: geniriclaw workspace <-> CLI skill dirs.

    Syncs between geniriclaw workspace, ~/.claude/skills, ~/.codex/skills,
    and ~/.gemini/skills.
    ...
    all_dirs: dict[str, Path] = {"geniriclaw": paths.skills_dir, **cli_dirs}
    ...
    # Priority order: geniriclaw > claude > codex > gemini
    priority = ("geniriclaw", "claude", "codex", "gemini")
...
def _iter_bundled_entries(paths: GeniriclawPaths) -> list[tuple[Path, Path]]:
...
def sync_bundled_skills(paths: GeniriclawPaths, *, docker_active: bool = False) -> None:
    """Sync bundled skills from the package into the geniriclaw workspace.

    Creates symlinks (or copies when *docker_active*) from
    ``~/.geniriclaw/workspace/skills/<name>`` to the package's
    ``_home_defaults/workspace/skills/<name>`` so bundled skills
    stay up-to-date with the installed geniriclaw version.
...
def cleanup_geniriclaw_links(paths: GeniriclawPaths) -> int:
    """Remove symlinks created by geniriclaw in CLI skill directories.

    Only removes symlinks whose resolved target is under the geniriclaw workspace
    ...
    for cli_dir in _cli_skill_dirs().values():
        ...
        for entry in cli_dir.iterdir():
            ...
            if any(_is_under(resolved, root) for root in managed_roots):
                entry.unlink()
                removed += 1
                logger.info("Removed geniriclaw skill link: %s", entry)

    if removed:
        logger.info("Cleaned up %d geniriclaw skill link(s) from CLI directories", removed)
    return removed
