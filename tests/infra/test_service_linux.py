...
from geniriclaw.infra.service_linux import (
    _SERVICE_NAME,
...
class TestGenerateServiceUnit:
    def test_contains_binary_path(self) -> None:
        unit = _generate_service_unit("/usr/local/bin/geniriclaw")
        assert "ExecStart=/usr/local/bin/geniriclaw" in unit

    def test_has_restart_policy(self) -> None:
        unit = _generate_service_unit("geniriclaw")
    ...
    def test_includes_all_nvm_bins(self, tmp_path: Path) -> None:
        ...
        with patch("geniriclaw.infra.service_linux.Path.home", return_value=tmp_path):
            unit = _generate_service_unit("geniriclaw")
...
class TestIsServiceAvailable:
    @patch("geniriclaw.infra.service_linux.shutil.which", return_value="/usr/bin/systemctl")
    def test_available_with_systemctl(self, _mock: MagicMock) -> None:
        assert is_service_available() is True

    @patch("geniriclaw.infra.service_linux.shutil.which", return_value=None)
...
class TestIsServiceRunning:
    @patch("geniriclaw.infra.service_linux._run_systemctl")
    @patch("geniriclaw.infra.service_linux.is_service_installed", return_value=True)
    @patch("geniriclaw.infra.service_linux._has_systemd", return_value=True)
...
class TestStartService:
    @patch("geniriclaw.infra.service_linux._run_systemctl")
    @patch("geniriclaw.infra.service_linux._has_systemd", return_value=False)
    ...
    @patch("geniriclaw.infra.service_linux._run_systemctl")
    @patch("geniriclaw.infra.service_linux.is_service_installed", return_value=False)
    @patch("geniriclaw.infra.service_linux._has_systemd", return_value=True)
    def test_start_not_installed(
        ...
        console.print.assert_called_with(
            "[dim]Service not installed. Run [bold]geniriclaw service install[/bold].[/dim]"
        )
...
class TestStopService:
    @patch("geniriclaw.infra.service_linux._run_systemctl")
    @patch("geniriclaw.infra.service_linux.is_service_running", return_value=True)
...
class TestUninstallService:
    ...
    @patch("geniriclaw.infra.service_linux._run_systemctl")
    @patch("geniriclaw.infra.service_linux._service_path")
    @patch("geniriclaw.infra.service_linux.is_service_installed", return_value=True)
    @patch("geniriclaw.infra.service_linux._has_systemd", return_value=True)
    def test_uninstall(
        self,
