...
_GENIRICLAW_MOUNT = "/geniriclaw"
_CONTAINER_WS = f"{_GENIRICLAW_MOUNT}/workspace"
_MOUNT_PREFIX = "/mnt"
...
class DockerManager:
    ...
    def __init__(self, config: DockerConfig, paths: GeniriclawPaths) -> None:
        self._config = config
        self._paths = paths
    ...
    async def _start_container(self, name: str, image: str) -> bool:
        # Always mount the root geniriclaw home, even when called from a sub-agent.
        ...
        # like /geniriclaw/agents/<name>/workspace.
        geniriclaw_home = self._paths.geniriclaw_home
        if geniriclaw_home.parent.name == "agents":
            geniriclaw_home = geniriclaw_home.parent.parent
        ...
        cmd: list[str] = [
            ...
            "-w",
            _CONTAINER_WS,
            # Mount the ENTIRE ~/.geniriclaw so the CLI sees all framework files.
            "-v",
            f"{geniriclaw_home}:{_GENIRICLAW_MOUNT}",
            "-e",
            f"GENIRICLAW_HOME={_GENIRICLAW_MOUNT}",
    ...
    def _env_secret_flags(self) -> list[str]:
        """Return ``-e`` flags for user secrets from ``~/.geniriclaw/.env``."""
