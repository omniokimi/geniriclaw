...
class RulesSelector:
    ...
    def __init__(self, paths: GeniriclawPaths) -> None:
    ...
    def deploy_rules(self) -> None:
        """Auto-discover and deploy all rule files to ~/.geniriclaw/.

        Scans _home_defaults/ for directories with RULES templates, selects
        the best variant for current auth state, and deploys to ~/.geniriclaw/
        ...
        for template_dir in template_dirs:
            ...
            dst_dir = self._paths.geniriclaw_home / rel_path
    ...
    def _remove_files_by_name(self, filename: str) -> int:
        """Remove all files with given name in ~/.geniriclaw/.
        ...
        """
        cron_tasks_path = self._paths.geniriclaw_home / "workspace" / "cron_tasks"
        removed_count = 0
        for file_path in self._paths.geniriclaw_home.rglob(filename):
            ...
            try:
                ...
                removed_count += 1
                logger.debug(
                    "Removed stale file: %s", file_path.relative_to(self._paths.geniriclaw_home)
                )
            except OSError:
