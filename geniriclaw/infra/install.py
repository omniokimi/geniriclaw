"""Detect how geniriclaw was installed (pipx, pip, or dev/source)."""
...
def detect_install_mode() -> InstallMode:
    ...
    Returns:
        ``"pipx"`` -- installed via ``pipx install geniriclaw``
        ``"pip"``  -- installed via ``pip install geniriclaw`` (from PyPI)
