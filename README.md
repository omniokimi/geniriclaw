<p align="center">
  <img src="https://raw.githubusercontent.com/igorfabrus/geniriclaw/main/geniriclaw/messenger/telegram/geniriclaw_images/logo_text.png" alt="geniriclaw" width="100%" />
</p>
...
<p align="center">
  <a href="https://pypi.org/project/geniriclaw/"><img src="https://img.shields.io/pypi/v/geniriclaw?color=blue" alt="PyPI" /></a>
  <a href="https://pypi.org/project/geniriclaw/"><img src="https://img.shields.io/pypi/pyversions/geniriclaw?v=1" alt="Python" /></a>
  <a href="https://github.com/igorfabrus/geniriclaw/blob/main/LICENSE"><img src="https://img.shields.io/github/license/igorfabrus/geniriclaw" alt="License" /></a>
</p>
...
If you want to control Claude Code, Google's Gemini CLI, or OpenAI's Codex CLI via Telegram, build automations, or manage multiple agents easily — geniriclaw is the right tool for you. Additional messengers (Matrix, and more to come) are supported via a modular plugin system.

geniriclaw runs on your machine and sends simple console commands as if you were typing them yourself, so you can use your active subscriptions (Claude Max, etc.) directly. No API proxying, no SDK patching, no spoofed headers. Just the official CLIs, executed as subprocesses, with all state kept in plain JSON and Markdown under `~/.geniriclaw/`.

<p align="center">
  <img src="https://raw.githubusercontent.com/igorfabrus/geniriclaw/main/docs/images/geniriclaw-start.jpeg" alt="geniriclaw /start screen" width="49%" />
  <img src="https://raw.githubusercontent.com/igorfabrus/geniriclaw/main/docs/images/geniriclaw-quick-actions.jpeg" alt="geniriclaw quick action buttons" width="49%" />
</p>

## Quick start

```bash
pipx install geniriclaw
geniriclaw
```
...
For Matrix support: `geniriclaw install matrix` — see [Matrix setup guide](docs/matrix-setup.md).

Detailed setup: [`docs/installation.md`](docs/installation.md)

## How chats work

geniriclaw gives you multiple ways to interact with your coding agents. Each level builds on the previous one.
...
All chats share the same `~/.geniriclaw/` workspace — same tools, same memory, same files. The only thing isolated is the conversation context.

> **Telegram note:** The Bot API has no method to list existing forum topics.
> geniriclaw learns topic names from `forum_topic_created` and `forum_topic_edited`
> events — pre-existing topics show as "Topic #N" until renamed.
> This is a Telegram limitation, not a geniriclaw limitation.
...
```bash
geniriclaw agents add codex-agent    # creates a new bot (needs its own BotFather token)
```
...
Sub-agents live under `~/.geniriclaw/agents/<name>/` with their own workspace, tools, and memory — fully isolated from the main agent.
...
| **Workspace** | `~/.geniriclaw/` | Shared with main | Shared with parent chat | Shared with parent agent | Own under `~/.geniriclaw/agents/` |
| **Config** | Main config | Shared with main | Shared with parent chat | Shared with parent agent | Own config (heartbeat, timeouts, model, ...) |
| **Setup** | Automatic | Create group + enable topics | `/session <prompt>` | Automatic or "delegate this" | Telegram: `geniriclaw agents add`; Matrix: `agents.json` / tool scripts |

### How it all fits together

```text
~/.geniriclaw/                          ← shared workspace (tools, memory, files)
  │
...
|---|---|---|---|---|
| **Telegram** | primary | Live message edits | Inline keyboards | `pip install geniriclaw` |
| **Matrix** | supported | Segment-based (new messages) | Emoji reactions | `geniriclaw install matrix` |
...
in a new sub-package and registering it — the rest of geniriclaw works without changes.
...
geniriclaw uses a dual-allowlist model. Every message must pass both checks.
...
geniriclaw's UI (commands, status messages, onboarding) is available in multiple languages:
...
geniriclaw onboarding       # Re-run setup wizard
geniriclaw reset            # Full reset + onboarding
geniriclaw stop             # Stop bot
geniriclaw restart          # Restart bot
geniriclaw upgrade          # Upgrade and restart
geniriclaw status           # Runtime status
geniriclaw help             # CLI overview
geniriclaw uninstall        # Remove bot + workspace

geniriclaw service install  # Install as background service
geniriclaw service status   # Show service status
geniriclaw service start    # Start service
geniriclaw service stop     # Stop service
geniriclaw service logs     # View service logs
geniriclaw service uninstall

geniriclaw docker enable    # Enable Docker sandbox
geniriclaw docker rebuild   # Rebuild sandbox container
geniriclaw docker mount /p  # Add host mount
geniriclaw docker extras    # List optional sandbox packages

geniriclaw agents list      # List configured sub-agents
geniriclaw agents add NAME  # Add a sub-agent
geniriclaw agents remove NAME

geniriclaw api enable       # Enable WebSocket API (beta)
geniriclaw api disable      # Disable WebSocket API

geniriclaw install matrix   # Install Matrix transport extra
geniriclaw install api      # Install API/PyNaCl extra
```

`geniriclaw agents add` currently scaffolds Telegram sub-agents interactively. Matrix
...
Other projects manipulate SDKs or patch CLIs and risk violating provider terms of service. geniriclaw simply runs the official CLI binaries as subprocesses — nothing more.
...
geniriclaw runs official provider CLIs and does not impersonate provider clients. Validate your own compliance requirements before unattended automation.
...
```bash
git clone https://github.com/igorfabrus/geniriclaw.git
cd geniriclaw
python -m venv .venv && source .venv/bin/activate
pip install -e ".[dev]"
pytest && ruff format . && ruff check . && mypy geniriclaw
```
