...
Fastest end-to-end mental model for geniriclaw.
...
- optional sub-agent stacks from `~/.geniriclaw/agents.json`
...
## 7) Key runtime files (`~/.geniriclaw`)
...
Sub-agent home: `~/.geniriclaw/agents/<name>/` with its own config/workspace/session files.

## 8) Where to read code first

1. `geniriclaw/__main__.py` (entrypoint + config/load/run)
2. `geniriclaw/cli_commands/` (actual CLI subcommand logic)
3. `geniriclaw/multiagent/supervisor.py` (always-on runtime wrapper)
4. `geniriclaw/messenger/telegram/app.py` + `messenger/telegram/startup.py` (Telegram), `geniriclaw/messenger/matrix/bot.py` (Matrix)
5. `geniriclaw/orchestrator/core.py` + `orchestrator/lifecycle.py`
6. `geniriclaw/bus/*` (unified delivery/injection)
7. `geniriclaw/tasks/hub.py` + `tasks/registry.py`
8. `geniriclaw/cli/service.py` and provider wrappers
...
- `geniriclaw`
- `geniriclaw status|stop|restart|upgrade|uninstall|onboarding|reset|help`
- `geniriclaw service ...`
- `geniriclaw docker ...` (includes `extras`, `extras-add`, `extras-remove` for optional AI/ML packages)
- `geniriclaw api ...`
- `geniriclaw agents ...` (`add` currently scaffolds Telegram sub-agents; Matrix sub-agents are added via `agents.json` or tool scripts)
- `geniriclaw install <extra>` (`matrix`, `api`)
...
- `geniriclaw service logs` follows `journalctl` on Linux and tails file logs on macOS/Windows
