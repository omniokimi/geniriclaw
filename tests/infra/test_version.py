...
from geniriclaw.infra.version import (
    VersionInfo,
    _parse_version,
    check_pypi,
    fetch_changelog,
...
class TestGetCurrentVersion:
    ...
    def test_returns_installed_version(self) -> None:
        with patch("geniriclaw.infra.version.importlib.metadata.version", return_value="1.5.0"):
            assert get_current_version() == "1.5.0"

    def test_returns_fallback_when_not_installed(self) -> None:
        with patch(
            "geniriclaw.infra.version.importlib.metadata.version",
...
class TestCheckPypi:
    ...
    async def test_returns_version_info_when_update_available(self) -> None:
        ...
        with (
            patch("geniriclaw.infra.version.get_current_version", return_value="1.0.0"),
            patch("geniriclaw.infra.version.aiohttp.ClientSession", mock),
        ):
    ...
    async def test_returns_none_on_http_error(self) -> None:
        ...
        with patch("geniriclaw.infra.version.aiohttp.ClientSession", mock):
            result = await check_pypi()
    ...
    async def test_fresh_mode_sets_cache_bust_headers(self) -> None:
        ...
        with (
            patch("geniriclaw.infra.version.get_current_version", return_value="1.0.0"),
            patch("geniriclaw.infra.version.aiohttp.ClientSession", mock_session_cm),
        ):
