...
4. One of these messaging transports:
   ...
   - **Matrix**: install Matrix support first (`geniriclaw install matrix` or `pip install \"geniriclaw[matrix]\"`), then provide homeserver URL, user ID, and password/access token
...
- writes config and seeds `~/.geniriclaw/`.
...
- On Linux, geniriclaw maps UID/GID to avoid root-owned files.
- If Docker setup fails at startup, geniriclaw logs warning and falls back to host execution.

Docker CLI shortcuts:

```bash
geniriclaw docker enable
geniriclaw docker disable
geniriclaw docker rebuild
geniriclaw docker mount /path/to/project
geniriclaw docker unmount /path/to/project
geniriclaw docker mounts
geniriclaw docker extras
geniriclaw docker extras-add <id>
geniriclaw docker extras-remove <id>
```
...
- run `geniriclaw docker mounts` to inspect effective mapping and broken paths.
...
- after changing extras, run `geniriclaw docker rebuild` to apply. Build output is streamed live to the terminal.
...
`geniriclaw api enable` requires PyNaCl (used for E2E encryption). If it is missing:

```bash
# pipx install
pipx inject geniriclaw PyNaCl

# pip install
pip install "geniriclaw[api]"
```
...
- token is generated and persisted by `geniriclaw api enable` (runtime also generates it on API start if still empty).
...
```bash
geniriclaw service install
```

Manage:

```bash
geniriclaw service status
geniriclaw service start
...
- Linux: `systemd --user` service `~/.config/systemd/user/geniriclaw.service`
- macOS: Launch Agent `~/Library/LaunchAgents/dev.geniri.claw.plist`
- Windows: Task Scheduler task `geniriclaw`
...
- service install prefers `pythonw.exe -m geniriclaw` (no visible console window),
...
- Linux: live `journalctl --user -u geniriclaw -f`
- macOS/Windows: recent lines from `~/.geniriclaw/logs/agent.log` (fallback newest `*.log`)
...
- use `/upgrade` or `pipx upgrade geniriclaw`
...
2. run `geniriclaw status`
3. inspect `~/.geniriclaw/logs/agent.log`
...
rm -rf ~/.geniriclaw  # optional data removal
```
