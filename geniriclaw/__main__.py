"""Entry point: python -m geniriclaw."""
...
from geniriclaw.cli_commands.service import cmd_service as _cmd_service
from geniriclaw.cli_commands.status import (
    print_status as _print_status,
)
from geniriclaw.cli_commands.status import (
    print_usage as _print_usage,
)
from geniriclaw.config import (
...
def load_config() -> AgentConfig:
    ...
    Resolution order:
    1. ``~/.geniriclaw/config/config.json`` (canonical location)
    ...
    if first_start:
        ...
        if example.is_file():
            ...
            defaults.pop("api", None)  # Beta: only written by `geniriclaw api enable`
...
async def run_bot(config: AgentConfig) -> int:
    ...
    """
    paths = resolve_paths(geniriclaw_home=config.geniriclaw_home)
    ...
    acquire_lock(pid_file=paths.geniriclaw_home / "bot.pid", kill_existing=True)
    ...
    try:
        ...
        release_lock(pid_file=paths.geniriclaw_home / "bot.pid")
    return exit_code
...
def _cmd_setup(verbose: bool) -> None:
    ...
    from geniriclaw.cli.init_wizard import run_onboarding, run_smart_reset

    _stop_bot()
    paths = resolve_paths()
    if _is_configured():
        run_smart_reset(paths.geniriclaw_home)
...
def _default_action(verbose: bool) -> None:
    ...
    if not _is_configured():
        from geniriclaw.cli.init_wizard import run_onboarding
