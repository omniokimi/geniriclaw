...
from geniriclaw.cli.types import AgentResponse
from geniriclaw.config import AgentConfig
from geniriclaw.errors import CLIError, CronError, SessionError, StreamError, WorkspaceError
...
async def test_create_with_authenticated_provider(
    ...
    with (
        patch(
            "geniriclaw.orchestrator.lifecycle.resolve_paths",
            return_value=paths,
        ),
        patch(
            "geniriclaw.cli.auth.check_all_auth",
            return_value={"claude": claude_auth, "codex": codex_auth},
        ),
        patch(
            "geniriclaw.orchestrator.observers.watch_rule_files",
...
async def test_handle_heartbeat_delegates_to_flow(orch: Orchestrator) -> None:
    with patch(
        "geniriclaw.orchestrator.core.heartbeat_flow",
        new_callable=AsyncMock,
