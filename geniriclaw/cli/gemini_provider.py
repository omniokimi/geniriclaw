...
from geniriclaw.cli.auth import gemini_api_key_mode_selected
...
from geniriclaw.cli.gemini_events import extract_result_text, extract_text, parse_gemini_stream_line
from geniriclaw.cli.gemini_utils import (
    create_system_prompt_file,
    find_gemini_cli,
    find_gemini_cli_js,
)
from geniriclaw.cli.stream_events import ResultEvent, StreamEvent, SystemInitEvent
from geniriclaw.cli.types import CLIResponse
from geniriclaw.config import NULLISH_TEXT_VALUES
...
# Must match ``_GENIRICLAW_MOUNT`` in ``geniriclaw.infra.docker``.
_CONTAINER_GENIRICLAW = "/geniriclaw"
...
class GeminiCLI(BaseCLI):
    ...
    def _inject_config_gemini_api_key(self, env: dict[str, str]) -> None:
        """Inject GEMINI_API_KEY from geniriclaw config when API-key auth mode is active."""
        ...
        logger.debug("Injected GEMINI_API_KEY from geniriclaw config for Gemini API key mode")
    ...
    def _create_system_prompt_path(self) -> str | None:
        ...
        In Docker mode the file is written to ``~/.geniriclaw/tmp/`` which is
        ...
        if self._config.docker_container:
            tmp_dir = resolve_paths().geniriclaw_home / "tmp"
    ...
    def _host_to_container_path(host_path: str) -> str | None:
        """Translate a host path under ``~/.geniriclaw/`` to its container mount."""
        prefix = str(resolve_paths().geniriclaw_home)
        if host_path.startswith(prefix):
            return _CONTAINER_GENIRICLAW + host_path[len(prefix) :].replace("\\", "/")
        return None
