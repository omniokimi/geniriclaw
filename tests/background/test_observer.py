...
from geniriclaw.background.models import BackgroundResult, BackgroundSubmit
from geniriclaw.background.observer import MAX_TASKS_PER_CHAT, BackgroundObserver
from geniriclaw.cli.param_resolver import TaskExecutionConfig
from geniriclaw.cron.execution import OneShotExecutionResult
from geniriclaw.infra.task_runner import TaskResult
...
def _make_paths(tmp_path: Path) -> GeniriclawPaths:
    ...
    paths = GeniriclawPaths(
        geniriclaw_home=tmp_path / "home", home_defaults=fw / "workspace", framework_root=fw
    )
...
def _make_observer(paths: GeniriclawPaths, timeout: float = 300.0) -> BackgroundObserver:
...
@pytest.fixture
async def observer(paths: GeniriclawPaths) -> AsyncIterator[BackgroundObserver]:
...
class TestSubmit:
    async def test_returns_task_id(self, observer: BackgroundObserver) -> None:
        ...
        with patch(
            "geniriclaw.background.observer.run_oneshot_task",
...
class TestExecution:
    async def test_success_delivers_result(self, observer: BackgroundObserver) -> None:
        ...
        with patch("geniriclaw.background.observer.run_oneshot_task", return_value=result):
