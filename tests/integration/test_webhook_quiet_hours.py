...
from geniriclaw.config import AgentConfig, HeartbeatConfig, WebhookConfig
from geniriclaw.webhook.manager import WebhookManager
from geniriclaw.webhook.models import WebhookEntry
...
def _add_hook(mgr: WebhookManager, paths: GeniriclawPaths, **overrides: Any) -> WebhookEntry:
    defaults: dict[str, Any] = {
        "id": "test-hook",
        "title": "Test Hook",
        "description": "Testing",
...
async def test_webhook_ignores_heartbeat_quiet_hours(tmp_path: Path) -> None:
    ...
    with patch("geniriclaw.cron.execution.build_cmd", return_value=None):
