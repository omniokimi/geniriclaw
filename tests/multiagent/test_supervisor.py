...
from geniriclaw.multiagent.supervisor import (
...
def main_config(tmp_path: Path) -> AgentConfig:
    """Create a main config with tmp_path as geniriclaw_home."""
    return AgentConfig(
        geniriclaw_home=str(tmp_path),
...
class TestStartupFailures:
    ...
    async def test_internal_api_start_failure_is_propagated(
        ...
        with (
            patch(
                "geniriclaw.multiagent.internal_api.InternalAgentAPI.start",
                new_callable=AsyncMock,
                return_value=False,
            ),
            patch("geniriclaw.multiagent.supervisor.AgentStack.create", new_callable=AsyncMock),
...
class TestOnAgentsChanged:
    ...
    async def test_restarts_on_token_change(
        ...
        old_config = AgentConfig(
            telegram_token="old-token", geniriclaw_home=str(tmp_path / "agents/sub1")
        )
...
class TestStartSubAgent:
    ...
    async def test_handles_creation_failure(self, supervisor: AgentSupervisor) -> None:
        ...
        with patch(
            "geniriclaw.multiagent.supervisor.AgentStack.create",
...
class TestStopAll:
    ...
    async def test_cancellation_unblocks_start(self, supervisor: AgentSupervisor) -> None:
        ...
        with (
            ...
            patch(
                "geniriclaw.multiagent.shared_knowledge.SharedKnowledgeSync",
            ) as mock_sks_cls,
        ):
            mock_sks = MagicMock()
