...
_DOCKER_MOUNT = "/geniriclaw"


def _resolve_container_path(path: Path) -> Path:
    """Translate Docker container paths to host paths.

    Inside the sandbox container ``~/.geniriclaw`` is mounted at ``/geniriclaw``.
    CLI output references container-side paths like
    ``/geniriclaw/workspace/output_to_user/file.png`` which don't exist on the
    ...
    return geniriclaw_home / relative
