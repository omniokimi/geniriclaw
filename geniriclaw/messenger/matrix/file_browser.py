...
from geniriclaw.files.browser import BROWSER_EXCLUDED_NAMES, list_directory
...
def format_file_listing(paths: GeniriclawPaths, subdir: str = "") -> str:
    """Format a text listing of the workspace directory structure.

    Args:
        paths: Resolved workspace paths.
        subdir: Optional subdirectory relative to ``geniriclaw_home`` to list.
...
def _format_overview(paths: GeniriclawPaths) -> str:
...
def _format_subdir(paths: GeniriclawPaths, subdir: str) -> str:
    ...
    display_path = f"~/.geniriclaw/{subdir}"
