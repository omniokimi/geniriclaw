...
geniriclaw supports multiple messaging transports. The `transport` config
...
### `geniriclaw` entry (`geniriclaw/__main__.py`)
...
1. resolve paths and set `GENIRICLAW_HOME` for main agent
...
- shared registry: `~/.geniriclaw/tasks.json`
- folders: `~/.geniriclaw/workspace/tasks/<task_id>/`
...
- task tools receive `GENIRICLAW_CHAT_ID` and optional `GENIRICLAW_TOPIC_ID`
...
Source: `geniriclaw/_home_defaults/`.
