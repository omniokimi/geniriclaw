...
from geniriclaw.bus.bus import MessageBus
from geniriclaw.bus.lock_pool import LockPool
from geniriclaw.commands import BOT_COMMANDS as _COMMAND_DEFS
from geniriclaw.commands import MULTIAGENT_SUB_COMMANDS as _MA_SUB_DEFS
from geniriclaw.config import AgentConfig
from geniriclaw.files.allowed_roots import resolve_allowed_roots
from geniriclaw.i18n import t
from geniriclaw.infra.restart import EXIT_RESTART, consume_restart_marker
from geniriclaw.infra.updater import UpdateObserver
from geniriclaw.infra.version import VersionInfo, get_current_version
from geniriclaw.log_context import set_log_context
from geniriclaw.messenger.notifications import NotificationService
from geniriclaw.messenger.telegram.callbacks import (
    edit_selector_response,
    mark_button_choice,
    parse_ns_callback,
)
from geniriclaw.messenger.telegram.chat_tracker import ChatRecord, ChatTracker
from geniriclaw.messenger.telegram.file_browser import (
    file_browser_start,
    handle_file_browser_callback,
    is_file_browser_callback,
)
from geniriclaw.messenger.telegram.formatting import markdown_to_telegram_html
from geniriclaw.messenger.telegram.handlers import (
    handle_abort,
    handle_abort_all,
    handle_command,
    handle_interrupt,
...
from geniriclaw.messenger.telegram.media import (
    has_media,
...
from geniriclaw.messenger.telegram.message_dispatch import (
    NonStreamingDispatch,
    StreamingDispatch,
    run_non_streaming_message,
    run_streaming_message,
)
from geniriclaw.messenger.telegram.middleware import (
    MQ_PREFIX,
    AuthMiddleware,
    SequentialMiddleware,
)
from geniriclaw.messenger.telegram.sender import SendRichOpts, send_rich
from geniriclaw.messenger.telegram.sender import (
    send_files_from_text as _send_files_from_text,
)
from geniriclaw.messenger.telegram.topic import (
    TopicNameCache,
    get_session_key,
    get_thread_id,
)
from geniriclaw.messenger.telegram.typing import TypingContext as _TypingContext
from geniriclaw.messenger.telegram.welcome import (
...
from geniriclaw.multiagent.bus import AsyncInterAgentResult
from geniriclaw.session.key import SessionKey
from geniriclaw.tasks.models import TaskResult
from geniriclaw.text.response_format import SEP, fmt
...
_WELCOME_IMAGE = Path(__file__).resolve().parent / "geniriclaw_images" / "welcome.png"
_CAPTION_LIMIT = 1024
...
def _rebuild_commands() -> None:
    ...
    from geniriclaw.commands import get_bot_commands, get_multiagent_sub_commands
...
class TelegramBot:
    ...
    def __init__(
        ...
        *,
        ...
        from geniriclaw.messenger.telegram.transport import TelegramTransport
    ...
    def file_roots(self, paths: GeniriclawPaths) -> list[Path] | None:
    ...
    async def _on_startup(self) -> None:
        from geniriclaw.messenger.telegram.startup import run_startup
    ...
    async def _send_join_notification(self, chat_id: int) -> None:
        ...
        if not text:
            return
        from geniriclaw.messenger.telegram.sender import _send_text_chunks
    ...
    async def _show_welcome(self, message: Message) -> None:
        ...
        from geniriclaw.cli.auth import check_all_auth
            ...
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="GitHub", url="https://github.com/igorfabrus/geniriclaw"
                    ),
                    InlineKeyboardButton(
                        text="Changelog",
                        url="https://github.com/igorfabrus/geniriclaw/releases",
                    ),
                ],
                [InlineKeyboardButton(text="PyPI", url="https://pypi.org/project/geniriclaw/")],
            ],
        )
        await send_rich(
            self._bot,
    ...
    async def _on_showfiles(self, message: Message) -> None:
        """Handle /showfiles: interactive file browser for ~/.geniriclaw."""
    ...
    async def _on_forum_topic_created(self, message: Message) -> None:
        ...
        from geniriclaw.messenger.telegram.topic import get_topic_name_from_message
    ...
    async def _on_session(self, message: Message) -> None:
        ...
        try:
            if session_followup:
                ...
                from geniriclaw.orchestrator.core import NamedSessionRequest
    ...
    async def _on_restart(self, message: Message) -> None:
        if self._config.group_mention_only and not self._is_addressed(message):
            return
        from geniriclaw.infra.restart import write_restart_sentinel

        chat_id = message.chat.id
        paths = self._orch.paths
        sentinel = paths.geniriclaw_home / "restart-sentinel.json"
    ...
    async def _route_special_callback(
        ...
        from geniriclaw.orchestrator.selectors.model_selector import is_model_selector_callback

        if is_model_selector_callback(data):
            await self._handle_model_selector(key, message_id, data)
            return True

        from geniriclaw.orchestrator.selectors.cron_selector import is_cron_selector_callback
    ...
    async def _route_prefix_callback(
        ...
        from geniriclaw.orchestrator.selectors.session_selector import is_session_selector_callback
        from geniriclaw.orchestrator.selectors.task_selector import is_task_selector_callback
    ...
    async def _handle_model_selector(self, key: SessionKey, message_id: int, data: str) -> None:
        ...
        from geniriclaw.orchestrator.selectors.model_selector import handle_model_callback
    ...
    async def _handle_cron_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from geniriclaw.orchestrator.selectors.cron_selector import handle_cron_callback
    ...
    async def _handle_session_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from geniriclaw.orchestrator.selectors.session_selector import handle_session_callback
    ...
    async def _handle_task_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from geniriclaw.orchestrator.selectors.task_selector import handle_task_callback
    ...
    async def _handle_ns_callback(
        ...
        async with self._sequential.get_lock(key.lock_key):
            if self._config.streaming.enabled:
                from geniriclaw.orchestrator.flows import named_session_streaming

                result = await named_session_streaming(self._orch, key, session_name, label)
            else:
                from geniriclaw.orchestrator.flows import named_session_flow
    ...
    async def on_async_interagent_result(self, result: AsyncInterAgentResult) -> None:
        ...
        from geniriclaw.bus.adapters import from_interagent_result
    ...
    async def on_task_result(self, result: TaskResult) -> None:
        ...
        from geniriclaw.bus.adapters import from_task_result
    ...
    async def on_task_question(
        ...
        from geniriclaw.bus.adapters import from_task_question
    ...
    async def _handle_webhook_wake(self, chat_id: int, prompt: str) -> str | None:
        ...
        from geniriclaw.bus.envelope import LockMode
        ...
        from geniriclaw.bus.adapters import from_webhook_wake
    ...
    async def _on_update_available(self, info: VersionInfo) -> None:
        ...
        from geniriclaw.messenger.telegram.upgrade_handler import on_update_available
    ...
    async def _handle_upgrade_callback(
        ...
        from geniriclaw.messenger.telegram.upgrade_handler import handle_upgrade_callback
    ...
    async def _watch_restart_marker(self) -> None:
        ...
        paths = self._orch.paths
        marker = paths.geniriclaw_home / "restart-requested"
        try:
            while True:
