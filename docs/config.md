...
Runtime config file: `~/.geniriclaw/config/config.json`.

Seed source: `<repo>/config.example.json` (source checkout) or packaged fallback `geniriclaw/_config_example.json` (installed mode).

## Config Creation

Primary path: `geniriclaw onboarding` (interactive wizard) writes `config.json` with user-provided values merged into `AgentConfig` defaults.

## Load & Merge Behavior

Config is merged in two places:

1. `geniriclaw/__main__.py::load_config()`
...
2. `geniriclaw/workspace/init.py::_smart_merge_config()`
...
- `geniriclaw api enable` writes the `api` block (including generated token) into `config.json`.

## External API Secrets (`~/.geniriclaw/.env`)
...
## `AgentConfig` (`geniriclaw/config.py`)
...
| `geniriclaw_home` | `str` | `"~/.geniriclaw"` | Runtime home root |
...
| `user_id` | `str` | `""` | Bot user ID (e.g. `@geniriclaw:matrix.org`) |
...
| `store_path` | `str` | `"matrix_store"` | E2EE key store directory, relative to `geniriclaw_home` |

Notes:

- first successful login persists credentials to `~/.geniriclaw/<store_path>/credentials.json` (mode `0o600`), not back into `config.json`
...
- `~/.geniriclaw/cron_jobs.json` (`CronJob`)
- `~/.geniriclaw/webhooks.json` (`WebhookEntry`, `cron_task` mode)
...
| `image_name` | `str` | `"geniriclaw-sandbox"` | Docker image name |
| `container_name` | `str` | `"geniriclaw-sandbox"` | Docker container name |
...
`Orchestrator.create()` calls `DockerManager.setup()` when enabled. If setup fails, geniriclaw logs warning and falls back to host execution.
...
- updates are typically managed via `geniriclaw docker mount|unmount`
- changing mounts requires bot restart (or `geniriclaw docker rebuild`) to affect container run flags

### `extras`

Optional AI/ML packages installed into the Docker sandbox image at build time. Each entry is an ID from the extras registry (`geniriclaw/infra/docker_extras.py`).
...
Managed via `geniriclaw docker extras-add|extras-remove` or during onboarding wizard. Changes require `geniriclaw docker rebuild` to take effect.
...
| `token` | `str` | `""` | Bearer/WebSocket auth token (generated by `geniriclaw api enable`, with runtime generation fallback on API start) |
...
- `docker`, `api`, `webhooks`
- `geniriclaw_home`, `log_level`, `gemini_api_key`, `timeouts`, `tasks`

Restart classification is computed from `AgentConfig` top-level schema fields.

## Model Resolution

`ModelRegistry` (`geniriclaw/config.py`):
...
`resolve_user_timezone(configured)` in `geniriclaw/config.py`:
...
Path: `~/.geniriclaw/config/codex_models.json`.
...
Path: `~/.geniriclaw/config/gemini_models.json`.
...
Path: `~/.geniriclaw/agents.json`.

Top-level JSON array of `SubAgentConfig` objects. Each entry defines a sub-agent that runs alongside the main agent.

Managed via:

- `geniriclaw agents add <name>` (interactive CLI, currently Telegram-focused)
- `geniriclaw agents remove <name>` (CLI)
...
- `geniriclaw_home = ~/.geniriclaw/agents/<name>/`
