...
from geniriclaw.infra.version import VersionInfo, _parse_version, check_pypi
    ...
    *,
    ...
    if mode == "pipx":
        ...
        if target_version is None and not force_reinstall and sys.platform != "win32":
            return ["pipx", "upgrade", "--force", "geniriclaw"]
        # `pipx runpip` upgrades inside the venv.  On Windows this is
        # required because `pipx upgrade` tries to overwrite the global
        # geniriclaw.exe which the running process holds locked.
        spec = f"geniriclaw=={target_version}" if target_version else "geniriclaw"
        cmd = ["pipx", "runpip", "geniriclaw", "install", "--upgrade", "--no-cache-dir"]
        if force_reinstall:
...
async def _perform_upgrade_impl(
    *,
    ...
    if mode == "pipx" and normalized_target is not None and sys.platform != "win32":
        fallback_cmd = ["pipx", "upgrade", "--force", "geniriclaw"]
...
async def get_installed_version() -> str:
    ...
    proc = await asyncio.create_subprocess_exec(
        sys.executable,
        "-c",
        "from importlib.metadata import version; print(version('geniriclaw'))",
        stdout=asyncio.subprocess.PIPE,
...
def write_upgrade_sentinel(
    ...
    *,
    ...
    from geniriclaw.infra.atomic_io import atomic_bytes_save
