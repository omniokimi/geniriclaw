...
from geniriclaw.workspace.init import inject_runtime_environment
from geniriclaw.workspace.paths import GeniriclawPaths, resolve_paths
from geniriclaw.workspace.skill_sync import cleanup_geniriclaw_links, sync_bundled_skills, sync_skills
...
def _docker_skill_resync(paths: GeniriclawPaths) -> None:
...
async def create_orchestrator(
    ...
    *,
    ...
    if agent_name == "main":
        os.environ["GENIRICLAW_HOME"] = str(paths.geniriclaw_home)
    ...
    from geniriclaw.cli.auth import AuthStatus, check_all_auth
...
async def start_api_server(
    ...
    try:
        ...
        logger.warning(
            "API server enabled but PyNaCl is not installed. Install with: pip install geniriclaw[api]"
        )
        return

    if not config.api.token:
        from geniriclaw.config import update_config_file_async
...
async def shutdown(orch: Orchestrator) -> None:
    ...
    await asyncio.to_thread(cleanup_geniriclaw_links, orch._paths)
