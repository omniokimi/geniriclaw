"""Docker management CLI subcommands (``geniriclaw docker ...``)."""
...
def _parse_docker_mount_arg(args: list[str]) -> str | None:
    ...
    Expects the form: ``geniriclaw docker mount <path>`` where *args* is
    ``sys.argv[1:]`` (no ``geniriclaw`` prefix).  Non-flag positionals are
...
def print_docker_help() -> None:
    ...
    table.add_column()
    table.add_row("geniriclaw docker rebuild", "Remove container & image, rebuild on next start")
    table.add_row("geniriclaw docker enable", "Enable Docker sandboxing")
    table.add_row("geniriclaw docker disable", "Disable Docker sandboxing")
    table.add_row("geniriclaw docker mount <path>", "Mount a host directory into the sandbox")
    table.add_row("geniriclaw docker unmount <path>", "Remove a mounted directory")
    table.add_row("geniriclaw docker mounts", "List all mounted directories")
    table.add_row("geniriclaw docker extras", "List available and installed extras")
    table.add_row("geniriclaw docker extras-add <id>", "Add an extra package")
    table.add_row("geniriclaw docker extras-remove <id>", "Remove an extra package")
    _console.print(
...
def docker_set_enabled(*, enabled: bool) -> None:
    ...
    if not isinstance(docker, dict):
        data["docker"] = docker = {}
    from geniriclaw.infra.json_store import atomic_json_save
...
def docker_rebuild() -> None:
    ...
    result = docker_read_config()
    container = "geniriclaw-sandbox"
    image = "geniriclaw-sandbox"
...
def docker_mount(args: list[str]) -> None:
    ...
    from geniriclaw.infra.docker import resolve_mount_target
...
def docker_extras_list() -> None:
    ...
    from geniriclaw.infra.docker_extras import extras_for_display
...
def docker_extras_add(args: list[str]) -> None:
    ...
    from geniriclaw.infra.docker_extras import DOCKER_EXTRAS_BY_ID
...
def docker_extras_remove(args: list[str]) -> None:
    ...
    from geniriclaw.infra.docker_extras import DOCKER_EXTRAS, DOCKER_EXTRAS_BY_ID
...
def cmd_docker(args: list[str]) -> None:
    """Handle 'geniriclaw docker <subcommand>'."""
