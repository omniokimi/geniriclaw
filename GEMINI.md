...
geniriclaw is a Telegram bot that routes chat input to official provider CLIs (`claude`, `codex`, `gemini`), streams responses back to Telegram, persists per-chat state, and runs cron/webhook/heartbeat automation in-process.
...
- `GeniriclawPaths` (`workspace/paths.py`) is the single source of truth for paths.
...
- Skill sync spans `~/.geniriclaw/workspace/skills`, `~/.claude/skills`, `~/.codex/skills`, `~/.gemini/skills`.
  - normal mode: links
  - Docker mode: managed copies (`.geniriclaw_managed` marker)
...
`geniriclaw service logs` behavior:
...
| Command | Effect |
|---|---|
| `geniriclaw` | Start bot (runs onboarding if needed) |
| `geniriclaw stop` | Stop bot and Docker container |
| `geniriclaw restart` | Restart bot |
| `geniriclaw upgrade` | Stop, upgrade, restart |
| `geniriclaw docker rebuild` | Stop bot, remove container & image, rebuilt on next start |
| `geniriclaw docker enable` | Set `docker.enabled = true` |
| `geniriclaw docker disable` | Stop container, set `docker.enabled = false` |
| `geniriclaw service install` | Install as background service |
| `geniriclaw service [sub]` | Service management (status/stop/logs/...) |

## Data Files (`~/.geniriclaw`)
