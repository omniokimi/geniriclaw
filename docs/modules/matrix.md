...
Matrix can be configured via the interactive setup wizard (`geniriclaw onboarding`) or manually in `config.json`.
...
- `geniriclaw agents add <name>` currently scaffolds Telegram sub-agents only
...
1. On first start, the bot logs in with the `password` and saves credentials to `~/.geniriclaw/<store_path>/credentials.json` (mode `0o600`)
