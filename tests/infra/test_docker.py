...
from geniriclaw.config import DockerConfig
from geniriclaw.workspace.paths import GeniriclawPaths


@pytest.fixture
def docker_paths(tmp_path: Path) -> GeniriclawPaths:
...
class TestDockerManager:
    ...
    async def test_setup_returns_none_when_docker_not_found(
        self, docker_config: DockerConfig, docker_paths: GeniriclawPaths
    ) -> None:
    ...
    async def test_mounts_full_geniriclaw_home(
        self, docker_config: DockerConfig, docker_paths: GeniriclawPaths
    ) -> None:
        """Verify run command mounts entire ~/.geniriclaw, not just workspace."""
        from geniriclaw.infra.docker import DockerManager
        ...
        # Full geniriclaw_home mounted at /geniriclaw
        assert f"{docker_paths.geniriclaw_home}:/geniriclaw" in run_str
        # Working dir is /geniriclaw/workspace
        assert "-w /geniriclaw/workspace" in run_str
        # GENIRICLAW_HOME env var set inside container
        assert "GENIRICLAW_HOME=/geniriclaw" in run_str
    ...
    async def test_uid_mapping_on_linux(
        ...
        with (
            patch("shutil.which", return_value="/usr/bin/docker"),
            patch.object(mgr, "_exec", side_effect=mock_exec),
            patch("geniriclaw.infra.docker._needs_uid_mapping", return_value=True),
    ...
    async def test_no_uid_mapping_on_macos(
        ...
        with (
            patch("shutil.which", return_value="/usr/bin/docker"),
            patch.object(mgr, "_exec", side_effect=mock_exec),
            patch("geniriclaw.infra.docker._needs_uid_mapping", return_value=False),
        ):
            await mgr.setup()
    ...
    async def test_sub_agent_mounts_root_geniriclaw_home(
        self, docker_config: DockerConfig, tmp_path: Path
    ) -> None:
        """Sub-agent container mounts ~/.geniriclaw (root), not ~/.geniriclaw/agents/test."""
        from geniriclaw.infra.docker import DockerManager

        root_home = tmp_path / ".geniriclaw"
        ...
        paths = GeniriclawPaths(
            geniriclaw_home=agent_home, home_defaults=fw / "workspace", framework_root=fw
        )
        ...
        assert f"{root_home}:/geniriclaw" in run_str
        assert f"{agent_home}:/geniriclaw" not in run_str
