...
from geniriclaw.commands import BOT_COMMANDS, MULTIAGENT_SUB_COMMANDS
...
from geniriclaw.messenger.commands import classify_command
from geniriclaw.messenger.matrix.buttons import ButtonTracker
from geniriclaw.messenger.matrix.credentials import login_or_restore
from geniriclaw.messenger.matrix.id_map import MatrixIdMap
from geniriclaw.messenger.matrix.sender import send_rich as matrix_send_rich
from geniriclaw.messenger.matrix.streaming import MatrixStreamEditor
from geniriclaw.messenger.matrix.typing import MatrixTypingContext
...
def _expand_marker(geniriclaw_home: str) -> Path:
    """Return the restart-marker path (sync, no I/O)."""
    return Path(geniriclaw_home).expanduser() / "restart-requested"
...
class MatrixBot:
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install 'geniriclaw[matrix]'"
            ) from None

        self._config = config
        self._agent_name = agent_name
        mx = config.matrix
        self._store_path = Path(config.geniriclaw_home).expanduser() / mx.store_path
        ...
        from geniriclaw.messenger.matrix.transport import MatrixTransport
    ...
    async def run(self) -> int:
        ...
        from geniriclaw.messenger.matrix.startup import run_matrix_startup
    ...
    async def _on_media(self, room: MatrixRoom | object, event: RoomMessageMedia | object) -> None:
        ...
        from geniriclaw.messenger.matrix.media import resolve_matrix_media
    ...
    async def _cmd_restart(
        ...
        from geniriclaw.infra.restart import EXIT_RESTART, write_restart_marker

        marker = _expand_marker(self._config.geniriclaw_home)
    ...
    async def _cmd_showfiles(
        ...
        from geniriclaw.messenger.matrix.file_browser import format_file_listing
    ...
    async def _send_selector_response(
        ...
        from geniriclaw.messenger.matrix.buttons import REACTION_DIGITS
        from geniriclaw.orchestrator.selectors.models import ButtonGrid
    ...
    async def _handle_button_callback(
        ...
        from geniriclaw.messenger.callback_router import route_callback
    ...
    async def _handle_upgrade_callback(self, room_id: str, data: str) -> None:
        ...
        if data.startswith("upg:cl:"):
            ...
            if not version:
                return
            from geniriclaw.infra.version import fetch_changelog
    ...
    async def _handle_ns_callback(self, room_id: str, key: SessionKey, data: str) -> None:
        ...
        from geniriclaw.messenger.telegram.callbacks import parse_ns_callback
