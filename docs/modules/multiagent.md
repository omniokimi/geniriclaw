...
Multi-agent runtime: run multiple independent geniriclaw agents in one process.
...
- workspace and files under `~/.geniriclaw/agents/<name>/`
...
- central log file (`~/.geniriclaw/logs/agent.log`)
- shared knowledge source (`~/.geniriclaw/SHAREDMEMORY.md`)
...
`SharedKnowledgeSync` watches `~/.geniriclaw/SHAREDMEMORY.md` and mirrors content into each agent's `MAINMEMORY.md` block.
...
- `geniriclaw agents`
- `geniriclaw agents list`
- `geniriclaw agents add <name>`
- `geniriclaw agents remove <name>`

`geniriclaw agents list` fetches live health from internal API when main bot is running.

Important CLI nuance:

- `geniriclaw agents add <name>` currently prompts for Telegram token/user/group data only.
