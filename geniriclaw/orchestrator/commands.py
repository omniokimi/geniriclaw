...
from geniriclaw.infra.version import check_pypi, get_current_version
from geniriclaw.orchestrator.registry import OrchestratorResult
from geniriclaw.orchestrator.selectors.cron_selector import cron_selector_start
from geniriclaw.orchestrator.selectors.model_selector import model_selector_start, switch_model
from geniriclaw.orchestrator.selectors.models import Button, ButtonGrid
from geniriclaw.orchestrator.selectors.session_selector import session_selector_start
from geniriclaw.orchestrator.selectors.task_selector import task_selector_start
from geniriclaw.text.response_format import SEP, fmt, new_session_text
from geniriclaw.workspace.loader import read_mainmemory
...
def _resolve_log_path(orch: Orchestrator) -> Path:
    ...
    log in the main geniriclaw home (parent of ``agents/<name>``).
    """
    log_path = orch.paths.logs_dir / "agent.log"
    if not log_path.exists():
        main_logs = orch.paths.geniriclaw_home.parent.parent / "logs" / "agent.log"
        if main_logs.exists():
            return main_logs
    return log_path
