...
from geniriclaw.cli.auth import AuthResult, AuthStatus
from geniriclaw.workspace.paths import GeniriclawPaths
from geniriclaw.workspace.rules_selector import RulesSelector


@pytest.fixture
def mock_paths(tmp_path: Path) -> GeniriclawPaths:
    """Create GeniriclawPaths with temp directories for testing."""
    home_defaults = tmp_path / "home_defaults"
    geniriclaw_home = tmp_path / "geniriclaw_home"

    # Create directory structure
    home_defaults.mkdir()
    geniriclaw_home.mkdir()
    ...
    paths = MagicMock(spec=GeniriclawPaths)
    paths.home_defaults = home_defaults
    paths.geniriclaw_home = geniriclaw_home

    return paths


def test_variant_selection_claude_only(mock_paths: GeniriclawPaths) -> None:
    ...
    with patch("geniriclaw.cli.auth.check_all_auth", return_value=auth):
        selector = RulesSelector(mock_paths)
        assert selector.get_variant_suffix() == "claude-only"


def test_variant_selection_codex_only(mock_paths: GeniriclawPaths) -> None:
...
def test_variant_selection_both(mock_paths: GeniriclawPaths) -> None:
...
def test_template_discovery(mock_paths: GeniriclawPaths) -> None:
...
def test_deploy_claude_only_no_agents_md(mock_paths: GeniriclawPaths) -> None:
    ...
    with patch("geniriclaw.cli.auth.check_all_auth", return_value=auth):
        ...
        config_claude = mock_paths.geniriclaw_home / "config" / "CLAUDE.md"
        assert config_claude.exists()
        assert "Claude Only Template" in config_claude.read_text()

        # Check that AGENTS.md was NOT created
        config_agents = mock_paths.geniriclaw_home / "config" / "AGENTS.md"
        assert not config_agents.exists()


def test_deploy_codex_only_with_agents_md(mock_paths: GeniriclawPaths) -> None:
...
def test_deploy_both_with_both_files(mock_paths: GeniriclawPaths) -> None:
...
def test_deploy_all_directories(mock_paths: GeniriclawPaths) -> None:
    ...
    with patch("geniriclaw.cli.auth.check_all_auth", return_value=auth):
        ...
        assert (mock_paths.geniriclaw_home / "config" / "CLAUDE.md").exists()
        assert (mock_paths.geniriclaw_home / "config" / "AGENTS.md").exists()
        assert (mock_paths.geniriclaw_home / "workspace" / "cron_tasks" / "CLAUDE.md").exists()
        assert (mock_paths.geniriclaw_home / "workspace" / "cron_tasks" / "AGENTS.md").exists()
        assert (
            mock_paths.geniriclaw_home / "workspace" / "tools" / "webhook_tools" / "CLAUDE.md"
        ).exists()
        assert (
            mock_paths.geniriclaw_home / "workspace" / "tools" / "webhook_tools" / "AGENTS.md"
        ).exists()


def test_fallback_to_static_template(mock_paths: GeniriclawPaths) -> None:
    ...
    with patch("geniriclaw.cli.auth.check_all_auth", return_value=auth):
        ...
        deployed = mock_paths.geniriclaw_home / "static_test" / "CLAUDE.md"
        assert deployed.exists()
        assert "Static Template" in deployed.read_text()


def test_skip_directory_without_templates(mock_paths: GeniriclawPaths) -> None:
...
def test_cleanup_removes_agents_md_when_only_claude(mock_paths: GeniriclawPaths) -> None:
    ...
    old_agents1 = mock_paths.geniriclaw_home / "config" / "AGENTS.md"
    # cron_tasks files are user-owned — they must survive cleanup
    cron_task_agents = mock_paths.geniriclaw_home / "workspace" / "cron_tasks" / "my-task" / "AGENTS.md"
...
def test_cleanup_removes_claude_md_when_only_codex(mock_paths: GeniriclawPaths) -> None:
    """Test that stale CLAUDE.md files are removed when only Codex is authenticated.

    Files inside workspace/cron_tasks/ are user-owned and must NOT be deleted.
    """
    old_claude1 = mock_paths.geniriclaw_home / "config" / "CLAUDE.md"
    cron_task_claude = mock_paths.geniriclaw_home / "workspace" / "cron_tasks" / "my-task" / "CLAUDE.md"
...
def test_cleanup_keeps_both_when_both_authenticated(mock_paths: GeniriclawPaths) -> None:
...
def test_variant_selection_gemini_only(mock_paths: GeniriclawPaths) -> None:
...
def test_variant_selection_claude_and_gemini(mock_paths: GeniriclawPaths) -> None:
...
def test_variant_selection_all_three(mock_paths: GeniriclawPaths) -> None:
...
def test_variant_selection_codex_and_gemini(mock_paths: GeniriclawPaths) -> None:
...
def test_deploy_with_gemini_creates_gemini_md(mock_paths: GeniriclawPaths) -> None:
    ...
    with patch("geniriclaw.cli.auth.check_all_auth", return_value=auth):
        ...
        gemini_md = mock_paths.geniriclaw_home / "config" / "GEMINI.md"
        assert gemini_md.exists()
        assert "Gemini Only Template" in gemini_md.read_text()

        # CLAUDE.md and AGENTS.md should NOT exist
        assert not (mock_paths.geniriclaw_home / "config" / "CLAUDE.md").exists()
        assert not (mock_paths.geniriclaw_home / "config" / "AGENTS.md").exists()


def test_deploy_all_three_providers(mock_paths: GeniriclawPaths) -> None:
    ...
    with patch("geniriclaw.cli.auth.check_all_auth", return_value=auth):
        ...
        config_dir = mock_paths.geniriclaw_home / "config"
        assert (config_dir / "CLAUDE.md").exists()
        assert (config_dir / "AGENTS.md").exists()
        assert (config_dir / "GEMINI.md").exists()


def test_cleanup_removes_gemini_md_when_not_authenticated(mock_paths: GeniriclawPaths) -> None:
    """Test that stale GEMINI.md files are removed when Gemini is not authenticated."""
    old_gemini = mock_paths.geniriclaw_home / "config" / "GEMINI.md"
