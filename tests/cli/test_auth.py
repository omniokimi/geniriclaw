...
from geniriclaw.cli.auth import (
    AuthResult,
    AuthStatus,
    check_claude_auth,
    check_codex_auth,
...
def _patch_claude_cli_fallback(monkeypatch: pytest.MonkeyPatch, *, logged_in: bool = False) -> None:
    ...
    import geniriclaw.cli.auth as _auth_mod
...
def test_check_gemini_auth_geniriclaw_config_key(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import geniriclaw.cli.auth as _auth_mod
    ...
    geniriclaw_config = tmp_path / ".geniriclaw" / "config" / "config.json"
    geniriclaw_config.parent.mkdir(parents=True)
    geniriclaw_config.write_text('{"gemini_api_key":"from-geniriclaw-config"}')

    result = check_gemini_auth()

    assert result.status == AuthStatus.AUTHENTICATED
    assert result.auth_file == geniriclaw_config


def test_check_gemini_auth_geniriclaw_config_null_string_ignored(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import geniriclaw.cli.auth as _auth_mod
    ...
    geniriclaw_config = tmp_path / ".geniriclaw" / "config" / "config.json"
    geniriclaw_config.parent.mkdir(parents=True)
    geniriclaw_config.write_text('{"gemini_api_key":"null"}')
