...
from geniriclaw.cli.param_resolver import (
    TaskOverrides,
    resolve_cli_config,
)
from geniriclaw.config import AgentConfig, reset_gemini_models, set_gemini_models
from geniriclaw.errors import GeniriclawError
...
def test_resolve_global_only(base_config: AgentConfig, codex_cache: CodexModelCache) -> None:
    ...
    assert result.working_dir == "~/geniriclaw"
...
def test_resolve_invalid_claude_model(
    ...
    with pytest.raises(GeniriclawError, match="Invalid Claude model"):
...
def test_resolve_invalid_codex_model(
    ...
    with pytest.raises(GeniriclawError, match="Invalid Codex model"):
        resolve_cli_config(base_config, codex_cache, task_overrides=overrides)
...
def test_resolve_gemini_invalid_against_discovered_models(
    ...
    with pytest.raises(GeniriclawError, match="Invalid Gemini model"):
        resolve_cli_config(base_config, codex_cache, task_overrides=overrides)
