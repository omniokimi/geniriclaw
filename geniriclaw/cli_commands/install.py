"""CLI command: geniriclaw install <extra>."""
...
def _install_extra(name: str) -> None:
    ...
    if mode == "pipx":
        cmd = [sys.executable, "-m", "pip", "install", f"geniriclaw[{name}]"]
    elif mode == "dev":
...
def print_install_help() -> None:
    ...
    Console().print(Panel(table, title="geniriclaw install <extra>"))


def cmd_install(args: list[str]) -> None:
    """Handle `geniriclaw install <extra>`."""
