...
geniriclaw routes chat input to official provider CLIs (`claude`, `codex`, `gemini`), streams responses back via Telegram or Matrix, persists session state, and runs cron/heartbeat/webhook/cleanup automation in-process. It also supports a direct WebSocket API transport with authenticated file upload/download.
...
16. `docs/modules/workspace.md` -- `~/.geniriclaw` seeding, rules sync, skill sync.
...
- `geniriclaw/__main__.py`: thin CLI entrypoint (dispatch) + config loading.
- `geniriclaw/cli_commands/`: concrete CLI subcommand implementations (`agents`, `docker`, `service`, `api`, `install`, lifecycle/status helpers).
- `geniriclaw/messenger/`: transport-agnostic protocols, capabilities, notifications, registry.
- `geniriclaw/messenger/telegram/`: aiogram handlers, auth/sequencing middleware, streaming dispatch, callback routing, group audit/chat tracking.
- `geniriclaw/messenger/matrix/`: matrix-nio handlers, segment streaming, reaction buttons, formatting.
- `geniriclaw/orchestrator/`: command registry, directives/hooks, normal + streaming + heartbeat flows, provider/session/task wiring.
- `geniriclaw/bus/`: central `MessageBus` + `Envelope` + `LockPool`.
- `geniriclaw/session/`: provider-isolated session state keyed by `SessionKey(transport, chat_id, topic_id)` plus named-session registry.
- `geniriclaw/tasks/`: shared background task delegation (`TaskHub`) and persistent task registry.
- `geniriclaw/api/`: WebSocket ingress (`/ws`) and HTTP file endpoints (`/files`, `/upload`).
- `geniriclaw/cli/`: Claude/Codex/Gemini wrappers, stream-event normalization, auth checks, model caches, process registry.
- `geniriclaw/cron/`, `webhook/`, `heartbeat/`, `cleanup/`: in-process automation observers.
- `geniriclaw/workspace/`: path source-of-truth, home defaults sync, rules deployment/sync, skill sync.
- `geniriclaw/multiagent/`: supervisor, inter-agent bus, internal localhost API bridge, shared-knowledge sync.
- `geniriclaw/infra/`: PID lock, restart/update state, Docker manager, service backends, observer/task utilities.
- `geniriclaw/infra/service_*.py`: platform-specific service installation, control, and log access.
