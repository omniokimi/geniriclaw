...
def _agents_path() -> Path:
    ...
    Sub-agents have GENIRICLAW_HOME = ~/.geniriclaw/agents/<name>/, so we navigate
    up to the main home. Main agent's GENIRICLAW_HOME points directly to ~/.geniriclaw/.
    """
    import os

    home = Path(os.environ.get("GENIRICLAW_HOME", str(Path.home() / ".geniriclaw")))
...
def _main_home() -> Path:
    """Resolve the main agent's GENIRICLAW_HOME."""
    import os

    home = Path(os.environ.get("GENIRICLAW_HOME", str(Path.home() / ".geniriclaw")))
    if home.name != ".geniriclaw" and (home.parent.parent / "config").is_dir():
