...
def detect_agent_name() -> str:
    ...
    Sub-agent tools live at ``~/.geniriclaw/agents/<name>/workspace/tools/task_tools/``.
    Main agent tools live at ``~/.geniriclaw/workspace/tools/task_tools/``.
    ...
    # Fallback to env var
    return os.environ.get("GENIRICLAW_AGENT_NAME", "main")
...
def post_json(url: str, body: dict[str, object], *, timeout: int = 300) -> dict[str, object]:
    ...
    try:
        ...
        print("Make sure the Geniriclaw bot is running with tasks enabled.", file=sys.stderr)
        sys.exit(1)
