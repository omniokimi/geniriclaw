...
from geniriclaw.workspace.skill_sync import (
    _MANAGED_MARKER,
    _clean_broken_links,
    _discover_skills,
    _ensure_copy,
...
def test_canonical_geniriclaw_priority(tmp_path: Path) -> None:
    geniriclaw = {"sk": tmp_path / "geniriclaw" / "sk"}
    claude = {"sk": tmp_path / "claude" / "sk"}
    codex = {"sk": tmp_path / "codex" / "sk"}
    for d in (geniriclaw["sk"], claude["sk"], codex["sk"]):
        d.mkdir(parents=True)
    result = _resolve_canonical("sk", geniriclaw, claude, codex)
    assert result == geniriclaw["sk"]
...
def _setup_three_dirs(
    tmp_path: Path,
) -> tuple[GeniriclawPaths, Path, Path]:
...
def test_sync_claude_to_geniriclaw(tmp_path: Path) -> None:
    paths, claude_skills, _ = _setup_three_dirs(tmp_path)
    _make_skill(claude_skills, "from-claude")
    with patch("geniriclaw.workspace.skill_sync._cli_skill_dirs") as mock:
...
def test_sync_codex_to_geniriclaw(tmp_path: Path) -> None:
...
def test_sync_geniriclaw_to_both(tmp_path: Path) -> None:
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    claude_skills.mkdir(parents=True, exist_ok=True)
    codex_skills.mkdir(parents=True, exist_ok=True)
    _make_skill(paths.skills_dir, "from-geniriclaw")
    with patch("geniriclaw.workspace.skill_sync._cli_skill_dirs") as mock:
        mock.return_value = {"claude": claude_skills, "codex": codex_skills}
        sync_skills(paths)
    for d in (claude_skills, codex_skills):
        link = d / "from-geniriclaw"
        assert link.is_symlink()
        assert link.resolve() == (paths.skills_dir / "from-geniriclaw").resolve()


def test_sync_gemini_to_geniriclaw(tmp_path: Path) -> None:
...
def test_sync_geniriclaw_to_all_three(tmp_path: Path) -> None:
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    gemini_home = tmp_path / "fake_home" / ".gemini"
    gemini_home.mkdir(parents=True)
    gemini_skills = gemini_home / "skills"
...
def test_sync_external_symlink(tmp_path: Path) -> None:
    ...
    with patch("geniriclaw.workspace.skill_sync._cli_skill_dirs") as mock:
        mock.return_value = {"claude": claude_skills, "codex": codex_skills}
        sync_skills(paths)
    geniriclaw_link = paths.skills_dir / "ext-skill"
    assert geniriclaw_link.is_symlink()
    assert geniriclaw_link.resolve() == external_real.resolve()
...
# ---------------------------------------------------------------------------
# Group 7: GeniriclawPaths.skills_dir property
# ---------------------------------------------------------------------------
...
def test_permission_error_logged(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with (
        patch("geniriclaw.workspace.skill_sync._cli_skill_dirs") as mock_dirs,
        patch("geniriclaw.workspace.skill_sync._create_dir_link", side_effect=OSError("denied")),
    ):
...
def test_sync_preserves_external_symlink(tmp_path: Path) -> None:
    ...
    # Geniriclaw also has a skill with the same name
    _make_skill(paths.skills_dir, "my-skill")
...
def test_sync_replaces_internal_symlink(tmp_path: Path) -> None:
    """Symlinks pointing inside sync dirs (geniriclaw-managed) can be updated."""
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    claude_skills.mkdir(parents=True, exist_ok=True)
    codex_skills.mkdir(parents=True, exist_ok=True)
    ...
    # Geniriclaw now has a real skill with the same name (higher priority)
    ...
    # Claude's symlink should now point to geniriclaw (higher priority canonical)
...
# ---------------------------------------------------------------------------
# Group 12: cleanup_geniriclaw_links
# ---------------------------------------------------------------------------


def test_cleanup_removes_geniriclaw_links(tmp_path: Path) -> None:
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    claude_skills.mkdir(parents=True, exist_ok=True)
    codex_skills.mkdir(parents=True, exist_ok=True)

    # Simulate geniriclaw-created symlinks (target under geniriclaw skills dir)
    _make_skill(paths.skills_dir, "from-geniriclaw")
    (claude_skills / "from-geniriclaw").symlink_to(paths.skills_dir / "from-geniriclaw")
    (codex_skills / "from-geniriclaw").symlink_to(paths.skills_dir / "from-geniriclaw")

    with patch("geniriclaw.workspace.skill_sync._cli_skill_dirs") as mock:
        mock.return_value = {"claude": claude_skills, "codex": codex_skills}
        removed = cleanup_geniriclaw_links(paths)

    assert removed == 2
    assert not (claude_skills / "from-geniriclaw").exists()
    assert not (codex_skills / "from-geniriclaw").exists()
