...
from geniriclaw.files.tags import (
    classify_mime,
    extract_file_paths,
    guess_mime,
    is_image_path,
...
class TestPathFromFileTag:
    def test_windows_path_slash_drive_form(self) -> None:
        with patch("geniriclaw.files.tags.is_windows", return_value=True):
    ...
    def test_posix_path_unchanged(self) -> None:
        with patch("geniriclaw.files.tags.is_windows", return_value=False):
    ...
    def test_docker_path_translated(self) -> None:
        with (
            patch.dict("os.environ", {"GENIRICLAW_HOME": "/home/user/.geniriclaw"}),
            patch("geniriclaw.files.tags.is_windows", return_value=False),
        ):
            path = path_from_file_tag("/geniriclaw/workspace/output_to_user/img.png")
        assert path == Path("/home/user/.geniriclaw/workspace/output_to_user/img.png")
    ...
    def test_docker_path_root(self) -> None:
        with (
            ...
            path = path_from_file_tag("/geniriclaw/sessions.json")
        assert path == Path("/home/user/.geniriclaw/sessions.json")

    def test_non_docker_path_not_translated(self) -> None:
        with patch("geniriclaw.files.tags.is_windows", return_value=False):
            path = path_from_file_tag("/home/user/.geniriclaw/workspace/file.txt")
        assert path == Path("/home/user/.geniriclaw/workspace/file.txt")
