...
from geniriclaw.config import AgentConfig
...
_LIFECYCLE = "geniriclaw.cli_commands.lifecycle"
_STATUS = "geniriclaw.cli_commands.status"
_SERVICE = "geniriclaw.cli_commands.service"
...
class TestLoadConfig:
    ...
    def test_creates_config_from_example(self, tmp_path: Path) -> None:
        from geniriclaw.__main__ import load_config

        home = tmp_path / ".geniriclaw"
        fw = tmp_path / "framework"
        fw.mkdir()
        example = {"telegram_token": "TEST", "provider": "claude"}
        (fw / "config.example.json").write_text(json.dumps(example))

        with patch("geniriclaw.__main__.resolve_paths") as mock_paths:
            paths = GeniriclawPaths(geniriclaw_home=home, home_defaults=fw / "workspace", framework_root=fw)
            mock_paths.return_value = paths
            with patch("geniriclaw.__main__.init_workspace"):
                config = load_config()
...
class TestRunTelegram:
    async def test_exits_on_missing_token(self, tmp_path: Path) -> None:
        from geniriclaw.__main__ import run_telegram

        config = AgentConfig(telegram_token="", geniriclaw_home=str(tmp_path))
    ...
    async def test_exits_on_placeholder_token(self, tmp_path: Path) -> None:
        ...
        config = AgentConfig(telegram_token="YOUR_TOKEN_HERE", geniriclaw_home=str(tmp_path))
        with pytest.raises(SystemExit):
            await run_telegram(config)
    ...
    async def test_exits_on_empty_allowed_users(self, tmp_path: Path) -> None:
        ...
        config = AgentConfig(
            telegram_token="valid:token", allowed_user_ids=[], geniriclaw_home=str(tmp_path)
        )
        with pytest.raises(SystemExit):
            await run_telegram(config)
    ...
    async def test_runs_bot_with_valid_config(self, tmp_path: Path) -> None:
        ...
        config = AgentConfig(
            telegram_token="valid:token", allowed_user_ids=[123], geniriclaw_home=str(tmp_path)
        )
        ...
        with (
            patch("geniriclaw.__main__.resolve_paths"),
            patch("geniriclaw.infra.pidlock.acquire_lock"),
            patch("geniriclaw.infra.pidlock.release_lock"),
            patch(
                "geniriclaw.multiagent.supervisor.AgentSupervisor",
...
def _make_paths(tmp_path: Path) -> GeniriclawPaths:
    ...
    return GeniriclawPaths(geniriclaw_home=home, home_defaults=fw / "workspace", framework_root=fw)


def _write_config(paths: GeniriclawPaths, data: dict[str, object]) -> None:
...
class TestIsConfiguredExtended:
    def test_unconfigured_with_placeholder_token(self, tmp_path: Path) -> None:
        ...
        with patch("geniriclaw.__main__.resolve_paths", return_value=paths):
            assert _is_configured() is False
...
class TestStopBot:
    def test_stop_kills_running_process(self, tmp_path: Path) -> None:
        from geniriclaw.cli_commands.lifecycle import stop_bot

        paths = _make_paths(tmp_path)
        paths.geniriclaw_home.mkdir(parents=True)
        pid_file = paths.geniriclaw_home / "bot.pid"
        pid_file.write_text("12345", encoding="utf-8")
        with (
            patch(f"{_LIFECYCLE}.resolve_paths", return_value=paths),
            patch("geniriclaw.infra.pidlock._is_process_alive", return_value=True),
            patch("geniriclaw.infra.pidlock._kill_and_wait") as mock_kill,
        ):
            stop_bot()
...
class TestUpgradeCli:
    def test_upgrade_with_pipx(self, tmp_path: Path) -> None:
        from geniriclaw.cli_commands.lifecycle import upgrade

        paths = _make_paths(tmp_path)
        paths.geniriclaw_home.mkdir(parents=True)
        with (
            patch("geniriclaw.infra.install.detect_install_mode", return_value="pipx"),
            patch(f"{_LIFECYCLE}.resolve_paths", return_value=paths),
            patch(
                "geniriclaw.infra.updater.perform_upgrade_pipeline",
                new=AsyncMock(return_value=(True, "9.9.9", "upgraded geniriclaw")),
            ) as mock_pipeline,
    ...
    def test_upgrade_with_pip(self, tmp_path: Path) -> None:
        ...
        paths.geniriclaw_home.mkdir(parents=True)
        with (
            patch("geniriclaw.infra.install.detect_install_mode", return_value="pip"),
            patch(f"{_LIFECYCLE}.resolve_paths", return_value=paths),
            patch(
                "geniriclaw.infra.updater.perform_upgrade_pipeline",
                new=AsyncMock(return_value=(True, "9.9.9", "installed geniriclaw-2.0.0")),
            ) as mock_pipeline,
            patch(f"{_LIFECYCLE}._re_exec_bot") as mock_exec,
            patch(f"{_LIFECYCLE}.stop_bot"),
        ):
    ...
    def test_upgrade_rejects_dev_mode(self) -> None:
        ...
        with (
            patch("geniriclaw.infra.install.detect_install_mode", return_value="dev"),
...
class TestReExecBot:
    def test_re_exec_uses_popen_on_posix(self) -> None:
        from geniriclaw.cli_commands.lifecycle import _re_exec_bot
        ...
        with (
            ...
            _re_exec_bot()
        mock_popen.assert_called_once_with([sys.executable, "-m", "geniriclaw"])
...
class TestStartBotRestart:
    ...
    def test_exit42_with_supervisor_exits(self) -> None:
        from geniriclaw.cli_commands.lifecycle import start_bot

        with (
            patch(f"{_LIFECYCLE}.resolve_paths"),
            patch("geniriclaw.logging_config.setup_logging"),
            patch("geniriclaw.__main__.load_config", return_value=self._mock_config()),
            patch(f"{_LIFECYCLE}.asyncio.run", side_effect=_mock_asyncio_run(42)),
            patch.dict("os.environ", {"GENIRICLAW_SUPERVISOR": "1"}),
            pytest.raises(SystemExit) as exc_info,
        ):
            start_bot()
...
class TestCountLogErrors:
    def test_counts_errors_in_log(self, tmp_path: Path) -> None:
        from geniriclaw.cli_commands.status import count_log_errors

        log_dir = tmp_path / "logs"
        log_dir.mkdir()
        (log_dir / "geniriclaw.log").write_text(
...
class TestUninstall:
    def test_uninstall_removes_workspace(self, tmp_path: Path) -> None:
        from geniriclaw.cli_commands.lifecycle import uninstall
        ...
        with (
            ...
            uninstall()
        assert not paths.geniriclaw_home.exists()
    ...
    def test_uninstall_cancelled(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            uninstall()
        assert paths.geniriclaw_home.exists()


class TestMainDispatch:
    def test_help_command(self) -> None:
        from geniriclaw.__main__ import main

        with (
            patch("sys.argv", ["geniriclaw", "help"]),
            patch("geniriclaw.__main__._print_usage") as mock_usage,
        ):
            main()
    ...
    def test_status_command(self) -> None:
        ...
        with (
            patch("sys.argv", ["geniriclaw", "status"]),
            patch("geniriclaw.__main__._cmd_status") as mock_status,
        ):
            main()
    ...
    def test_stop_command(self) -> None:
        ...
        with (
            patch("sys.argv", ["geniriclaw", "stop"]),
            patch("geniriclaw.__main__._stop_bot") as mock_stop,
        ):
            main()
    ...
    def test_default_starts_bot_when_configured(self) -> None:
        ...
        with (
            patch("sys.argv", ["geniriclaw"]),
            patch("geniriclaw.__main__._is_configured", return_value=True),
            patch("geniriclaw.__main__._start_bot") as mock_start,
        ):
            main()
    ...
    def test_default_runs_onboarding_when_unconfigured(self) -> None:
        ...
        with (
            patch("sys.argv", ["geniriclaw"]),
            patch("geniriclaw.__main__._is_configured", return_value=False),
            patch("geniriclaw.cli.init_wizard.run_onboarding", return_value=False) as mock_onboard,
            patch("geniriclaw.__main__._start_bot") as mock_start,
        ):
            main()
    ...
    def test_default_does_not_start_bot_when_service_installed(self) -> None:
        ...
        with (
            patch("sys.argv", ["geniriclaw"]),
            patch("geniriclaw.__main__._is_configured", return_value=False),
            patch("geniriclaw.cli.init_wizard.run_onboarding", return_value=True),
            patch("geniriclaw.__main__._start_bot") as mock_start,
        ):
            main()
    ...
    def test_upgrade_command(self) -> None:
        ...
        with (
            patch("sys.argv", ["geniriclaw", "upgrade"]),
            patch("geniriclaw.__main__._upgrade") as mock_upgrade,
        ):
            main()
    ...
    def test_onboarding_command(self) -> None:
        ...
        with (
            patch("sys.argv", ["geniriclaw", "onboarding"]),
            patch("geniriclaw.__main__._cmd_setup") as mock_setup,
        ):
            main()
    ...
    def test_reset_maps_to_setup(self) -> None:
        ...
        with (
            patch("sys.argv", ["geniriclaw", "reset"]),
            patch("geniriclaw.__main__._cmd_setup") as mock_setup,
        ):
            main()
        mock_setup.assert_called_once()
...
class TestSetupCommand:
    def test_setup_starts_bot_when_service_not_installed(self) -> None:
        from geniriclaw.__main__ import _cmd_setup

        with (
            patch("geniriclaw.__main__._stop_bot"),
            patch("geniriclaw.__main__.resolve_paths"),
            patch("geniriclaw.__main__._is_configured", return_value=False),
            patch("geniriclaw.cli.init_wizard.run_onboarding", return_value=False),
            patch("geniriclaw.__main__._start_bot") as mock_start,
        ):
            _cmd_setup(False)
        mock_start.assert_called_once_with(False)
...
class TestMainHelpers:
    def test_parse_service_subcommand_ignores_flags(self) -> None:
        from geniriclaw.cli_commands.service import _parse_service_subcommand
    ...
    def test_cmd_service_without_subcommand_prints_help(self) -> None:
        from geniriclaw.cli_commands.service import cmd_service
    ...
    def test_cmd_service_install_dispatches_backend(self) -> None:
        ...
        with patch("geniriclaw.infra.service.install_service") as mock_install:
    ...
    def test_cmd_service_status_dispatches_backend(self) -> None:
        ...
        with patch("geniriclaw.infra.service.print_service_status") as mock_status:
            cmd_service(["service", "status"])
        mock_status.assert_called_once()

    def test_print_usage_calls_status_when_configured(self) -> None:
        from geniriclaw.cli_commands.status import print_usage
    ...
    def test_print_usage_shows_not_configured_panel(self) -> None:
        ...
        with patch("geniriclaw.__main__._is_configured", return_value=False):
            print_usage()

    def test_cmd_status_prints_not_configured(self) -> None:
        from geniriclaw.__main__ import _cmd_status

        with patch("geniriclaw.__main__._is_configured", return_value=False):
            _cmd_status()

    def test_cmd_restart_stops_and_reexecs(self) -> None:
        from geniriclaw.cli_commands.lifecycle import cmd_restart
    ...
    def test_default_action_configured_starts(self) -> None:
        from geniriclaw.__main__ import _default_action
