...
from geniriclaw.cli.stream_events import StreamEvent
...
_CONTAINER_GENIRICLAW_MOUNT = "/geniriclaw"


def _to_container_path(host_path: Path, main_home: Path) -> str:
    """Map a host path under *main_home* to its container equivalent.

    The Docker container mounts the root geniriclaw home at ``/geniriclaw``.
    """
    rel = host_path.relative_to(main_home)
    if str(rel) == ".":
        return _CONTAINER_GENIRICLAW_MOUNT
    return f"{_CONTAINER_GENIRICLAW_MOUNT}/{rel.as_posix()}"
...
def docker_wrap(
    ...
    *extra_env* vars are injected as ``-e`` flags into ``docker exec``
    ...
    if config.docker_container:
        ...
        env_flags: list[str] = [
            "-e",
            f"GENIRICLAW_CHAT_ID={config.chat_id}",
            "-e",
            f"GENIRICLAW_TRANSPORT={config.transport}",
            "-e",
            f"GENIRICLAW_AGENT_NAME={config.agent_name}",
            "-e",
            f"GENIRICLAW_INTERAGENT_PORT={config.interagent_port}",
            "-e",
            f"GENIRICLAW_HOME={container_home}",
            "-e",
            f"GENIRICLAW_SHARED_MEMORY_PATH={container_shared}",
            "-e",
            "GENIRICLAW_INTERAGENT_HOST=host.docker.internal",
        ]
        if config.topic_id:
            env_flags += ["-e", f"GENIRICLAW_TOPIC_ID={config.topic_id}"]
        if extra_env:
