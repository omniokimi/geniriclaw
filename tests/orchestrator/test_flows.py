...
async def test_normal_warns_for_gemini_api_key_mode_without_geniriclaw_key(
    orch: Orchestrator,
) -> None:
    mock_execute = AsyncMock(return_value=_mock_response())
    object.__setattr__(orch._cli_service, "execute", mock_execute)
...
async def test_streaming_warns_for_gemini_api_key_mode_without_geniriclaw_key(
    orch: Orchestrator,
) -> None:
