"""Interactive file browser for the ~/.geniriclaw directory.

Renders the geniriclaw home directory as a navigable inline-keyboard tree.
...
from geniriclaw.files.browser import list_directory
from geniriclaw.i18n import t
from geniriclaw.security.paths import is_path_safe
...
async def file_browser_start(paths: GeniriclawPaths) -> tuple[str, InlineKeyboardMarkup]:
...
async def handle_file_browser_callback(
    ...
    if data.startswith(SF_FILE_PREFIX):
        ...
        abs_dir = (paths.geniriclaw_home / rel).resolve() if rel else paths.geniriclaw_home.resolve()
...
def _build_view(paths: GeniriclawPaths, rel: str) -> tuple[str, InlineKeyboardMarkup]:
    """Build the text + keyboard for a directory listing."""
    base = paths.geniriclaw_home.resolve()
    ...
    display_path = f"~/.geniriclaw/{rel}" if rel else "~/.geniriclaw/"
