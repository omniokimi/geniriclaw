...
from geniriclaw.cli.codex_cache import _FALLBACK_CODEX_MODELS, CodexModelCache
...
async def test_load_from_disk(tmp_path: Path) -> None:
    ...
    with patch("geniriclaw.cli.codex_cache.discover_codex_models", AsyncMock()) as mock_discover:
...
async def test_refresh_on_stale(tmp_path: Path, sample_models: list[CodexModelInfo]) -> None:
    ...
    with patch(
        "geniriclaw.cli.codex_cache.discover_codex_models",
